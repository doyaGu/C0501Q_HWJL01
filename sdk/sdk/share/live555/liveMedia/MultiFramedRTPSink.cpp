...
void MultiFramedRTPSink::packFramePayload() {
    if (fSource == NULL) return;
  ...
  if (numFrameBytesToUse == 0 && frameSize > 0) {
    ...
    unsigned char* frameStart = fOutBuf->curPtr();
      ...
      {    // Delay this amount of time:
        //printf("307\n");
        nextTask() = envir().taskScheduler().scheduleDelayedTask(1000, (TaskFunc*)retryLater, this);
        return;
      }
...
#if defined(CFG_DEMOD_ENABLE)
    // This modification only work for H264.
    unsigned char marker;
    fOutBuf->extract(&marker, 1, 1);
    if( (marker & 0x80) || fRecursiveCnt > 30 )
    {
        fRecursiveCnt = 0;
...
// The following is called after each delay between packet sends:
void MultiFramedRTPSink::retryLater(void* firstArg) {
  MultiFramedRTPSink* sink = (MultiFramedRTPSink*)firstArg;
  //printf("retry\n");
  sink->packFramePayload();
}
