...
static sdp_message_t *create_generic_sdp(const SalMediaDescription *desc)
{
    ...
    if(strlen(desc->ipcam_address) != 0)
    {
        sdp_message_i_info_set(local, -1, osip_strdup(desc->ipcam_address));
    }
...
static void add_line(sdp_message_t *msg, int lineno, const SalStreamDescription *desc){
	...
	if (strcmp(addr,sdp_message_c_addr_get(msg, -1, 0))!=0){
		bool_t inet6;
        int ret;
        char *nettype, *addrtype, *caddr;
		if (strchr(addr,':')!=NULL){
			inet6=TRUE;
		}else inet6=FALSE;
        nettype = osip_strdup ("IN");
        addrtype = inet6 ? osip_strdup ("IP6") : osip_strdup ("IP4");
        caddr = osip_strdup (addr);
		ret = sdp_message_c_connection_add (msg, lineno, nettype, addrtype, caddr, NULL, NULL);
        if (ret != OSIP_SUCCESS)
        {
            osip_free(nettype);
            osip_free(addrtype);
...
int sdp_to_media_description(sdp_message_t *msg, SalMediaDescription *desc){
	int i,j;
	const char *mtype,*proto,*port,*addr,*number,*url;
	sdp_bandwidth_t *sbw=NULL;
	
	addr=sdp_message_c_addr_get (msg, -1, 0);
	if (addr)
		strncpy(desc->addr,addr,sizeof(desc->addr));
    url = sdp_message_i_info_get(msg, -1);
    if (url)
    {
        strcpy(desc->ipcam_address, url);
    }
