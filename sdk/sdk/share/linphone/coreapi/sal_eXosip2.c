...
static bool_t b_call_redirected = FALSE;
...
Sal * sal_init(){
	...
	if (firsttime){
    #ifndef NDEBUG
        osip_trace_initialize_func (END_TRACE_LEVEL,&_osip_trace_func);
    #else
...
static void sdp_process(SalOp *h){
	...
	if (h->sdp_offering){	
        ...
        if(strlen(h->base.remote_media->ipcam_address) != 0)
            strcpy(h->result->ipcam_address, h->base.remote_media->ipcam_address);
...
int sal_call_terminate(SalOp *h){
	int err;
	if (h == NULL || h->terminated==TRUE) return -1;
...
static void call_ringing(Sal *sal, eXosip_event_t *ev){
    ...
    if(b_call_redirected == TRUE)
    {
        op->redirected = TRUE;
        b_call_redirected = FALSE;
    }
	else
		op->redirected = FALSE;
