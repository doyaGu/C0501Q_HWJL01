...
static void _tpInitSpec_vendor(void)
{
    ...
    gTpSpec.tpReadChipRegCnt    = (int)30;
#else
...
static int _tpParseRawPxy_vendor(struct ts_sample *s, unsigned char *buf)
{
	...
	{		
		...
		if(loopNum<lfnum)
			{
			smp = (struct ts_sample*)s;				
				
			if( (loopNum==1) && (lfnum==2) )
			{
				int id0 = (int)(buf[5]>>4);
				smp++;
				smp->pressure = 0;
	...
	if( buf[2] || ((buf[3]&0xf0)==0x40) )
	{
		...
		{
			s->y = (int)((((unsigned int)buf[3]<<8)&0x0F00) | ((unsigned int)buf[4]));
			s->x = (int)((((unsigned int)buf[5]<<8)&0x0F00) | ((unsigned int)buf[6]));
...
static int _tpProbeSample(struct ts_sample *samp, int nr)
{
	int ret=0;
	struct ts_sample *s=samp;
	struct ts_sample *tmpNxt=(struct ts_sample *)samp->next;
	...
	{
#ifdef TP_MULTI_FINGER_ENABLE
		int fn = (int)g_sample[0].finger;
		int sz = (int)sizeof(struct ts_sample);
		
        if( (fn >= 0) && (fn <= 5) )
        {
        	//printf("prb1:[%d,%d]:%d,%d,%d\n",g_sample[0].finger, g_sample[0].id, g_sample[0].pressure, g_sample[0].x, g_sample[0].y);
        	//if(g_sample[0].finger==2)	printf("prb2:[%d,%d]:%d,%d,%d\n",g_sample[1].finger, g_sample[1].id, g_sample[1].pressure, g_sample[1].x, g_sample[1].y);
        	
        	if(fn>2)	g_sample[0].finger = 2;	//force to limit 2 finfers currently
       	    ...
       	    if( (fn >= 2) && (samp->next!=NULL) )
       	    {
       	    	struct ts_sample *s1 = (struct ts_sample *)(samp->next);
       	    	struct ts_sample *s2 = (struct ts_sample *)&g_sample[1];
       	    	//tmpNxt = s1->next;
       	    	//memcpy((void *)s1, (void *)&s2, sizeof(struct ts_sample));
       	    	//s1->next = tmpNxt;
