...
static bool_t queue_packet(queue_t *q, int maxrqsz, mblk_t *mp, rtp_header_t *rtp, int *discarded)
{
    ...
    *discarded=0;
    ...
    while (q->q_mcount > maxrqsz)
    {
        ...
        if (mp!=NULL)
        {
            printf("rtp_putq: Queue is full. Discarding message with ts=%i\n",((rtp_header_t*)mp->b_rptr)->timestamp);
            freemsg(tmp);
            (*discarded)++;
        }
    }
...
void rtp_session_rtp_parse(RtpSession *session, mblk_t *mp, uint32_t local_str_ts, struct sockaddr *addr, socklen_t addrlen)
{
...
#if LOSS_SEQNUM_MSG    
    if(session->rtp.loc_port > 9000)
    {
        if(rtp->seq_number != (session->last_seqnum+1))
            printf("Enqueuing packet with ts=%d and seq=(%d). Loss packet seq(%d)~seq(%d)\n",rtp->timestamp, rtp->seq_number, session->last_seqnum+1, rtp->seq_number-1);
        session->last_seqnum = rtp->seq_number;
    }
#endif    
	...
	/*Disable RTP parse check timestamp flow because RTP sender performance too bad(multicast mode can not send out packet immediately), 
	so just bypass it!*/
