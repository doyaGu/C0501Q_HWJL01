...
#define RTP_SESSION_MULTICAST_FLAG (1<<14)
#endif
...
static uint32_t get_cur_timestamp(MSFilter * f, mblk_t *im)
{
	...
	if (im){
#if defined(ENABLE_VIDEO_MULTICAST)
		netts=mblk_get_timestamp_info(im);
#else		
...
#if defined(ENABLE_VIDEO_MULTICAST)	
	if (s->flags & RTP_SESSION_MULTICAST_FLAG)
		send_start = 1;
	else if(!(s->flags & RTP_SESSION_MULTICAST_FLAG) && send_start == 1)	
		return;
#endif
...
static void receiver_process(MSFilter * f)
{
	...
	if (d->session == NULL)
		return;
/*     if(d->starting){
        printf("%s %d\n",__FUNCTION__,__LINE__);//it's no use
    } */
	if (d->starting){
	...
	while ((m = rtp_session_recvm_with_ts(d->session, timestamp)) != NULL) {
        ...
        //mblk_set_timestamp_info_without_jitter_control(m, d->session->rtp.rcv_last_ts);
