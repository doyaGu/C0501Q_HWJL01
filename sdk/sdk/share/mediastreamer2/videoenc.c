...
void __register_ffmpeg_encoders_if_possible(void){
	ms_ffmpeg_check_init();
#ifdef ENABLE_GENERAL_PLAYER   	
	//if (avcodec_find_encoder(CODEC_ID_MPEG4))  // TODO: FIXME
...
#endif
#if defined(TS_CAM_ENABLE) && !defined(CFG_FFMPEG_H264_SW)
    do{
        extern MSFilterDesc ms_es_enc_desc;
        static uint32_t es_cam_initialized = 0;
        if( es_cam_initialized )    break;
        
        ms_filter_register(&ms_es_enc_desc);
        es_cam_initialized = 1;
    }while(0);
#endif
}
