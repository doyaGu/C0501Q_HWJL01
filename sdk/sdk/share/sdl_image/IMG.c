...
#if (CFG_JPEG_HW_ENABLE) && !(_MSC_VER)
    { "JPG", IMG_is_Ite_Jpg, IMG_Load_Ite_Jpg },
#else
...
SDL_Surface *IMG_LoadTyped_RW(SDL_RWops *src, int freesrc, const char *type)
{
	...
	for ( i=0; i < ARRAYSIZE(supported); ++i ) {
        ...
        // H/W jpg decode fail change to S/W decode
        if( strncmp(supported[i].type, "JPG", 3) == 0 && 
            image == NULL )
        {
            if( IMG_isJPG(src) )
                image = IMG_LoadJPG_RW(src);
        }
#endif
		if(freesrc)
			SDL_RWclose(src);
