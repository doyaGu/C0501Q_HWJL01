...
int siproxd_main (int argc, char *argv[])
{
   ...
   struct in_addr map_addr,   map2_addr;
...
/*get_interface_real_ip(IF_OUTBOUND, &map_addr);
get_interface_real_ip(IF_INBOUND, &map2_addr);
printf(" IF_OUTBOUND %s , IF_INBOUND %s \n",inet_ntoa(map_addr),inet_ntoa(map2_addr)); */
        proxy_set_in_indoor();
      /*
       * got input, process
       */
