...
int udpClinet2()
{
   ...
   struct sockaddr_in to_addr;
    ...
    int len;    
    char gStr[1024];
    struct sockaddr_in audiolink_addr; 
    int gAudiolinkLength;
	...
	to_addr.sin_port = htons(7078);
	to_addr.sin_addr.s_addr = inet_addr("192.168.190.253");	

    printf("udpClinet 2 ,192.168.190.253\n");



    sock=socket (PF_INET, SOCK_DGRAM, IPPROTO_UDP);
    if (sock < 0) {
        printf("udpClinet sock %s <0 \n",sock);
        return 0;
    }

   printf("udpClinet try to send ..... %d  \n\n",sock);

   gAudiolinkLength = sizeof(struct sockaddr_in);
   do {

        if (sendto(sock, "#", 1, 0, &to_addr, gAudiolinkLength) < 0) {
    ...
    memcpy(&my_addr.sin_addr.s_addr, &rtp_proxytable[gIndex].remote_ipaddr, sizeof(struct in_addr));

    my_addr.sin_port= htons(rtp_proxytable[gIndex].remote_port);


   printf("udpClinet try to send .....%s %d %d\n\n",utils_inet_ntoa(rtp_proxytable[gIndex].remote_ipaddr) ,rtp_proxytable[gIndex].rtp_tx_sock,rtp_proxytable[gIndex].remote_port);
   gAudiolinkLength = sizeof(struct sockaddr_in);
   do {

        if (sendto(rtp_proxytable[gIndex].rtp_tx_sock, "#", 1, 0, &my_addr, gAudiolinkLength) < 0) {
            printf("[Rtpproxy] sock %d Could not send datagram!!\n",rtp_proxytable[gIndex].rtp_tx_sock); 
        } else {
           printf("[Rtpproxy]send datagram!!\n"); 
        }
        usleep(2000000);
...
static int createUdpThread()
{
    ...
    pthread_create(&task, &attr, udpClinet2, NULL);
    return 0;
...
static void *rtpproxy_main(void *arg) {
   ...
   struct timezone tz ;
   char remip1[16], remip2[16],rxIP[16];
   char lclip1[16];
   struct sockaddr_in rTemp_addr; 
   int nTemp;
...
//      pthread_testcancel();
      ...
      for (i=0;(i<RTPPROXY_SIZE) && (num_fd>0);i++) {
         ...
         if ( (rtp_proxytable[i].rtp_con_rx_sock != 0) &&
            ...
            if (count > 0) {
               ...
               if (rtp_proxytable[i].rtp_con_tx_sock != 0) {
                  ...
                  strcpy(rxIP, utils_inet_ntoa(rtp_proxytable[i].remote_ipaddr) );

                  // Don't dejitter RTCP packets 
                  printf("sendto RTCP[%d] %d ,to %s:%d, %d, read(fd) %d , local port %d \n",i,rtp_proxytable[i].rtp_tx_sock,remip1,rtp_proxytable[i].remote_port,count,rtp_proxytable[i].rtp_con_rx_sock,rtp_proxytable[i].local_port);
          ...
          */
          if (i ==0){
            //printf("i %d %d  %d    \n\n",i,rtp_proxytable[i].rtp_rx_sock,FD_ISSET(rtp_proxytable[i].rtp_rx_sock, &fdset) );
         }
         if ( (rtp_proxytable[i].rtp_rx_sock != 0) &&
            FD_ISSET(rtp_proxytable[i].rtp_rx_sock, &fdset) ) {
            /* yup, have some data to send */
            num_fd--;
              //printf("RTProxy main read i %d  %d,%s:%d \n",i,rtp_proxytable[i].rtp_rx_sock,utils_inet_ntoa(rtp_proxytable[i].local_ipaddr),rtp_proxytable[i].local_port);
            /* read from sock rtp_proxytable[i].rtp_rx_sock */
            #if 0
                count = recvfrom(rtp_proxytable[i].rtp_rx_sock, rtp_buff, RTP_BUFFER_SIZE, 0, (struct sockaddr*)&rTemp_addr, &nTemp);
            #else
    ...
    #if 1       
/////
                  nTemp = rtp_proxytable[i].nSendto;
                  dst_addr.sin_family = AF_INET;
                  memcpy(&dst_addr.sin_addr.s_addr,
                         &rtp_proxytable[nTemp].remote_ipaddr,
                         sizeof(struct in_addr));
                  dst_addr.sin_port= htons(rtp_proxytable[nTemp].remote_port);
/////
            ...
            //if (gnPrint%25==0)    
                //printf("sendto [%d] %d ,%s:%d, %d, read(fd) %d , local port %d from %s \n",i,rtp_proxytable[nTemp].rtp_tx_sock,remip2,rtp_proxytable[nTemp].remote_port,count,rtp_proxytable[i].rtp_rx_sock,rtp_proxytable[i].local_port,inet_ntoa(rTemp_addr.sin_addr));
            #if 1
          sts = sendto(rtp_proxytable[nTemp].rtp_rx_sock, rtp_buff,
                               count, 0, (const struct sockaddr *)&dst_addr,
                               (socklen_t)sizeof(dst_addr));
    #else
             sts =  send(rtp_proxytable[nTemp].rtp_tx_sock, rtp_buff,count,0 );
            #endif
            ...
            sts = sendto(rtp_proxytable[i].dest_sock, rtp_buff,
                  ...
                  if (sts == -1) {
                     ...
                     if ((errno != ECONNREFUSED) && (errno != ENOBUFS)){
                        ...
                        rtp_proxytable[nTemp].remote_port, count, strerror(errno));
...
int rtp_relay_start_fwd (osip_call_id_t *callid, client_id_t client_id,
   ...
   static struct in_addr prev_used_local_addr ;   
   int num_ports;
   int i2, i, j;
   int sock, port;
   int sock_con;
   ...
   if (memcmp(&prev_used_local_addr ,&local_ipaddr,4)){
        printf("rtp_relay_start_fwd local addr %d \n",prev_used_port);
        prev_used_local_addr = local_ipaddr;
        prev_used_port = 0;
  }
   ...
   DEBUGC(DBCLASS_RTP,"1 rtp_relay_start_fwd: starting RTP proxy "
          "stream for: CallID=%s@%s [Client-ID=%s] (%s,%s) #=%i,local_ipaddr %s ,remote_ipaddr %s \n",
          ...
          media_stream_no,utils_inet_ntoa(local_ipaddr),utils_inet_ntoa(remote_ipaddr));
      ...
      if (rtp_proxytable[i].rtp_rx_sock &&
       ...
       // it changes real remote ip and port,so mark these code
         ...
         if (rtp_proxytable[i].remote_port != remote_port) {
            DEBUGC(DBCLASS_RTP,"2 RTP port number changed %i -> %i",
         ...
         DEBUGC(DBCLASS_RTP,"3 RTP stream already active idx=%i (remaddr=%s, "
   ...
   for (i2 = (prev_used_port - configuration.rtp_port_low + 1);
      ...
      if (j == RTPPROXY_SIZE) {
         ...
         if (sock) {
            ...
            if (sock_con) break;
            sts = close(sock);
            DEBUGC(DBCLASS_RTP,"4 closed socket %i [%i] for RTP stream because "
                               "cant get pair sts=%i",
                               sock, i, sts);
         } /* if sock */
      } /* if j */
...
printf("rtp_relay_start_fwd: addr=%s, port=%i, sock=%i, freeidx=%i, input data dejitter buffer=%i usec \n",utils_inet_ntoa(local_ipaddr), port, sock, freeidx, dejitter);

   DEBUGC(DBCLASS_RTP,"5 rtp_relay_start_fwd: addr=%s, port=%i, sock=%i, "
          "freeidx=%i, input data dejitter buffer=%i usec prev_used_port %d \n", 
          utils_inet_ntoa(local_ipaddr), port, sock, freeidx, dejitter,prev_used_port);
...
#if 1
    rtp_proxytable[freeidx].rtp_tx_sock = socket (PF_INET, SOCK_DGRAM, IPPROTO_UDP); /* RTP */
 /*
   gnUdp++;
   if (gnUdp==1){
        gnSock = rtp_proxytable[freeidx].rtp_tx_sock;
        printf("set gnSock %d \n\n",rtp_proxytable[freeidx].rtp_tx_sock );
        gIndex = freeidx;
    } */
#endif


    printf("add rtp relay To remote %s:%d,index %d  rx %d\n",utils_inet_ntoa(rtp_proxytable[freeidx].remote_ipaddr), rtp_proxytable[freeidx].remote_port,freeidx,sock);
   DEBUGC(DBCLASS_RTP,"6 add rtp_proxytable[%d]:rtp %d ,call %d,media %d,addr=%s, port=%i, sock=%i, "
          ", input data dejitter buffer=%i usec", 
          freeidx,  rtp_proxytable[freeidx].direction , rtp_proxytable[freeidx].call_direction, rtp_proxytable[freeidx].media_stream_no,  utils_inet_ntoa(local_ipaddr), port, sock,  dejitter);
...
static int match_socket (int rtp_proxytable_idx) {
   ...
   for (j=0;(j<RTPPROXY_SIZE);j++) {
     ...
     memcpy(&rtp_proxytable[rtp_proxytable_idx].dest_ipaddr,&rtp_proxytable[j].remote_ipaddr,sizeof(struct in_addr));
        ...
        rtp_proxytable[j].nSendto = rtp_proxytable_idx;
