...
extern int urlmap_index[];  /* URL mapping table  to register user index   */
...
static int if_a_string_is_a_valid_ipv4_address(const char *str)
{
    struct in_addr addr;
    int ret;
    int local_errno;
...
int  sip_find_register_user(sip_ticket_t *ticket) {
    int i,j;
    int nResult=0;
    ...
    for (i=0; i<URLMAP_SIZE; i++) {
        ...
        if (get_ip_by_host(urlmap[i].true_url->host, &tmp_addr) == STS_FAILURE) {
            ...
            printf("[sip proxy]find register user %s,%s,ip:%s \n",urlmap[i].true_url->host,urlmap[i].masq_url->host, utils_inet_ntoa(from->sin_addr));
            nResult = if_a_string_is_a_valid_ipv4_address(urlmap[i].masq_url->host);
            if (nResult == 0){
                urlmap[i].nRtp =0;
            } else if (nResult>0) {
 ...
 */
int  sip_find_direction_byIndex(sip_ticket_t *ticket, int idx) {
    int type;
    int i, sts;
    struct sockaddr_in *from;
    ...
    */
    if (urlmap[idx].active == 0){
        printf("[sip proxy] error table %d is not active #line %d\n",idx,__LINE__);
        return STS_FAILURE;
    }
    if (get_ip_by_host(urlmap[idx].true_url->host, &tmp_addr) == STS_FAILURE) {
        DEBUGC(DBCLASS_SIP, "sip_find_direction: cannot resolve host [%s]",
        urlmap[idx].true_url->host);
        return STS_FAILURE;        
    } else {
        DEBUGC(DBCLASS_SIP, "sip_find_direction: reghost:%s ip:%s",
        urlmap[idx].true_url->host, utils_inet_ntoa(from->sin_addr));
    ...
    if (type == DIRTYP_UNKNOWN) {
        ...
        */
        ...
        if (MSG_IS_REQUEST(ticket->sipmsg)) {
            ...
            if ((compare_url(request->to->url, urlmap[idx].masq_url)==STS_SUCCESS) ||(!MSG_IS_REGISTER(request) &&
            (compare_url(request->to->url, urlmap[idx].reg_url)==STS_SUCCESS))) {
                type=REQTYP_INCOMING;            
            }
        } else { 
            /* RESPONSE */
            /* incoming response ('from' == 'masq') || ('from' == 'reg') */
            if ((compare_url(response->from->url, urlmap[idx].reg_url)==STS_SUCCESS) || (compare_url(response->from->url, urlmap[idx].masq_url)==STS_SUCCESS)) {
    ...
    */
    if ((type == DIRTYP_UNKNOWN) && (MSG_IS_REQUEST(ticket->sipmsg))) {
        ...
        if ((compare_url(request->req_uri, urlmap[idx].masq_url)==STS_SUCCESS) ||(!MSG_IS_REGISTER(request) &&
        (compare_url(request->req_uri, urlmap[idx].reg_url)==STS_SUCCESS))) {
        type=REQTYP_INCOMING;
        }
    ...
    * inserted by myself
    */
    if ((type == DIRTYP_UNKNOWN) &&  (!osip_list_eol(&(response->vias), 0))) {
        ...
        sts=get_ip_by_host(via->host, &addr_via);
        if (sts == STS_FAILURE) {
            DEBUGC(DBCLASS_SIP, "sip_find_direction: cannot resolve VIA [%s]",via->host);
        } else {

            /* incoming response (1st via in list points to a registered UA) */
            sts=get_ip_by_host(urlmap[idx].true_url->host, &addr_myself);
            if (sts == STS_FAILURE) {
            ...
            if ((port_via<=0) || (port_via>65535))
                port_via=SIP_PORT;

            port_ua=0;
            if (urlmap[idx].true_url->port)
                port_ua=atoi(urlmap[idx].true_url->port);
            if ((port_ua<=0) || (port_ua>65535))
                port_ua=SIP_PORT;

            DEBUGC(DBCLASS_SIP, "sip_find_direction: checking for registered "
            "host [%s:%i] <-> [%s:%i]", urlmap[idx].true_url->host, port_ua,via->host, port_via);

            if ((memcmp(&addr_myself, &addr_via, sizeof(addr_myself))==0) && (port_via == port_ua)) {
                type=RESTYP_INCOMING;    
            }
    ...
    */
    if (type == DIRTYP_UNKNOWN) {
        ...
        if ((htonl(from->sin_addr.s_addr) == INADDR_LOOPBACK) ||(from->sin_addr.s_addr == tmp_addr.s_addr) ||
