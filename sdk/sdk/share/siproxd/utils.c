 ...
 */
int  get_interface_ip(int iface, struct in_addr *retaddr) {
   int sts=STS_FAILURE;

   if ((iface == IF_OUTBOUND) && 
      ...
      sts = get_interface_real_ip(iface, retaddr);
   }
 ...
 * STS_FAILURE if interface is DOWN or other problem
 */
int  get_interface_real_ip(int iface, struct in_addr *retaddr) {
   int sts=STS_FAILURE;
   char *tmp=NULL;

      if (iface == IF_INBOUND) {
         tmp = configuration.inbound_if;
      } else if (iface == IF_OUTBOUND) {
         tmp = configuration.outbound_if;
      }

   if (tmp && (strcmp(tmp, "")!=0)) {
      DEBUGC(DBCLASS_DNS, "fetching interface IP by INTERFACE [%i]", iface);
