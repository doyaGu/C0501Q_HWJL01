...
#include <pthread.h>
static pthread_mutex_t jpipe_write_mutex = PTHREAD_MUTEX_INITIALIZER;

#if 0 //!defined(WIN32) && !defined(__arc__)
...
int jpipe_write(jpipe_t * apipe, const void *buf, int count)
{
    ...
    pthread_mutex_lock(&jpipe_write_mutex);
    ret = send(apipe->pipes[1], buf, count, 0);
    pthread_mutex_unlock(&jpipe_write_mutex);
