...
static BOOL mkntfs_open_partition(ntfs_volume *vol)
{
    ...
    DISC_INTERFACE* interface = NULL;

    for (i = 0; discs[i].name != NULL && discs[i].interface != NULL; i++) {
        disc = &discs[i];
        if (strcmp(disc->name, opts.dev_name) == 0) {
    ...
    fd->startSector = opts.part_start_sect;
    fd->sectorSize = opts.sector_size;
    fd->sectorCount = opts.num_sectors;
    fd->cachePageCount = CACHE_DEFAULT_PAGE_COUNT;
    fd->cachePageSize = CACHE_DEFAULT_PAGE_SIZE;

	/*
	 * Allocate and initialize an ntfs device structure and attach it to
	 * the volume.
	 */
	vol->dev = ntfs_device_alloc(opts.dev_name, 0, &ntfs_device_default_io_ops, fd);
	if (!vol->dev) {
        ntfs_free(fd);
