...
static int vfw_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    codec = st->codec;
    #if _MSC_VER 
    codec->time_base.den = framerate_q.den;
    codec->time_base.num = framerate_q.num;
    #else
...
#define vfw_opt_str  0x6e74736300000000 // "ntsc";
static const AVOption options[] = {
    { "video_size", "A string describing frame size, such as 640x480 or hd720.", OFFSET(video_size), AV_OPT_TYPE_STRING, {0}, 0, 0, DEC },
    { "framerate", "", OFFSET(framerate), AV_OPT_TYPE_STRING, {(double)vfw_opt_str}, 0, 0, DEC },
    { NULL },
};

static const AVClass vfw_class = {
    "VFW indev",            // const char* class_name;
...
AVInputFormat ff_vfwcap_demuxer = {
    "vfwcap",               // const char *name;
    "VfW video capture",    // const char *long_name;
    sizeof(struct vfw_ctx), // int priv_data_size;
    NULL,                   // int (*read_probe)(AVProbeData *);
    vfw_read_header,        // int (*read_header)(struct AVFormatContext *, AVFormatParameters *ap);
    vfw_read_packet,        // int (*read_packet)(struct AVFormatContext *, AVPacket *pkt);
    vfw_read_close,         // int (*read_close)(struct AVFormatContext *);
#if FF_API_READ_SEEK
    NULL,                   // attribute_deprecated int (*read_seek)(struct AVFormatContext *, int stream_index, int64_t timestamp, int flags);
#endif
    NULL,                   // int64_t (*read_timestamp)(struct AVFormatContext *s, int stream_index, int64_t *pos, int64_t pos_limit);
    AVFMT_NOFILE,           // int flags;
...
#endif
    &vfw_class,             // const AVClass *priv_class;
