      ...
      if(stream->codec_type != AVMEDIA_TYPE_DATA){
        ...
        if(   stream->codec_type == AVMEDIA_TYPE_VIDEO
           ...
           && s->streams[i]->sample_aspect_ratio.den>0){
            ...
            AVRational dar = av_mul_q(s->streams[i]->sample_aspect_ratio, c);
            #else
...
static const AVCodecTag *avi_codec_tag[3] = 
{
    ff_codec_bmp_tags,
    ff_codec_wav_tags, 
    NULL,
...
AVOutputFormat ff_avi_muxer = {
    "avi",              // const char *name;
    "AVI format",       // const char *long_name;
    "video/x-msvideo",  // const char *mime_type;
    "avi",              // const char *extensions;
    sizeof(AVIContext), // int priv_data_size;
#if CONFIG_LIBMP3LAME_ENCODER
    CODEC_ID_MP3,       // enum CodecID audio_codec;
#else
    CODEC_ID_AC3,       // enum CodecID audio_codec;
#endif
    CODEC_ID_MPEG4,     // enum CodecID video_codec;
    avi_write_header,   // int (*write_header)(struct AVFormatContext *);
    avi_write_packet,   // int (*write_packet)(struct AVFormatContext *, AVPacket *pkt);
    avi_write_trailer,  // int (*write_trailer)(struct AVFormatContext *);
    AVFMT_VARIABLE_FPS, // int flags;
    NULL,               // void *dummy;
    NULL,               // int (*interleave_packet)(struct AVFormatContext *, AVPacket *out, AVPacket *in, int flush);
    avi_codec_tag,      // const struct AVCodecTag * const *codec_tag;
    ...
    NULL,               // int (*query_codec)(enum CodecID id, int std_compliance);
    NULL,               // void (*get_output_timestamp)(struct AVFormatContext *s, int stream, int64_t *dts, int64_t *wall);
