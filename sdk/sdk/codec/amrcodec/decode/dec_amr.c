...
**************************************************************************
*/
void Decoder_amr_init (Decoder_amrState **state)
{
  Decoder_amrState* s;
//  Word16 i;
...
**************************************************************************
*/
void Decoder_amr_reset (Decoder_amrState *state, enum Mode mode)
{
//  Word16 i;
...
//  Set_zero (state->old_exc, PIT_MAX + L_INTERPOL);
  Preset (0, state->old_exc, PIT_MAX + L_INTERPOL);

  if (mode != MRDTX)
//     Set_zero (state->mem_syn, M);
        Preset(0, state->mem_syn, M);
  ...
  if (mode != MRDTX) {
      ...
      Preset (0, state->excEnergyHist, 9);
  }

  // sis3830
  //for (i = 0; i < 9; i++)
  //   state->ltpGainHistory[i] = 0;
  Preset(0, state->ltpGainHistory, 9);
...
//      return;
  ...
  // sis3830 No Need to Memory-Free in our Implement
  /*
...
)
{
    ...
    if (((Word16)frame_type == RX_SPEECH_BAD) ||
    ...
    else if ((Word16)frame_type == RX_SPEECH_DEGRADED)
    {
       pdfi = 1;
    }
    ...
    if ((Word16)st->dtxDecoderState->dtxGlobalState == DTX)
    {
       st->state = 5;
       st->prev_bf = 0;
    }
    else if ((Word16)st->dtxDecoderState->dtxGlobalState == DTX_MUTE)
    {
       st->state = 5;
       st->prev_bf = 1;
    }
    ...
    //st->lsp_old[0] = lsp_new[0];
    //st->lsp_old[1] = lsp_new[1];
    //st->lsp_old[2] = lsp_new[2];
    //st->lsp_old[3] = lsp_new[3];
    //st->lsp_old[4] = lsp_new[4];
    //st->lsp_old[5] = lsp_new[5];
    //st->lsp_old[6] = lsp_new[6];
    //st->lsp_old[7] = lsp_new[7];
    //st->lsp_old[8] = lsp_new[8];
    //st->lsp_old[9] = lsp_new[9];
    ...
    *------------------------------------------------------------------------*/
    ...
    for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR)
    {
        ...
        *-------------------------------------------------------*/
        ...
        {
           temp = (Word16)((code[i - T0] * pit_sharp)>>15);
           code[i] += temp;
        }
        ...
        if (((Word16)mode != MR475) || (evenSubfr == 0))
        {
            st->sharp = gain_pit;
        ...
        if (pit_sharp > 16384)
        {
           for (i = 0; i < L_SUBFR; i++)
            {
               L_temp = (st->exc[i] * pit_sharp) >> 15;
               L_temp = (L_temp * gain_pit);
               if ((Word16)mode == MR122)
               {
                  L_temp >>= 1;
               }
               excp[i] = (Word16)((L_temp+0x00004000L)>>15);
            }
        }
         ...
         *-------------------------------------------------------*/
        if ((Word16)mode <= MR102) /* MR475, MR515, MR59, MR67, MR74, MR795, MR102*/
        {
           pitch_fac = gain_pit;
           tmp_shift = 1;
        }
        ...
        for (i = 0; i < L_SUBFR; i++)
        {
           ...
           L_temp = (st->exc[i] * pitch_fac);
           L_temp += (code[i] * gain_code);
           L_temp <<= tmp_shift;                   /* Q16 */
           st->exc[i] = (Word16)((L_temp+0x00004000L)>>15);
        }
        ...
        for (i=0; i < L_SUBFR; i++)
        {
            asm ("l.mac %0, %1" : : "r"((Word32)exc_enhanced[i]), "r"((Word32)exc_enhanced[i]));
        }
        ...
        for (i=0; i < L_SUBFR; i++)
        {
            LL_temp = LL_temp + ((Word32)exc_enhanced[i]) * ((Word32)exc_enhanced[i]);
        }
...
//      L_temp >>= 1;
        L_temp >>= (temp>>1) + 18;
//        L_temp >>= 2;       /* To cope with 16-bit and  */
        ...
        if (pit_sharp > 16384)
        {
           for (i = 0; i < L_SUBFR; i++)
           {
              excp[i] += exc_enhanced[i];
           }
           agc2 (exc_enhanced, excp, L_SUBFR);
           Overflow = 0;
           Syn_filt_de (Az, excp, &synth[i_subfr], L_SUBFR,
                     st->mem_syn, 0);
        }
        else
        {
           Overflow = 0;
           Syn_filt_de (Az, exc_enhanced, &synth[i_subfr], L_SUBFR,
                     st->mem_syn, 0);
        }
        ...
        if (Overflow != 0)    /* Test for overflow */
        {
           ...
           Syn_filt_de(Az, exc_enhanced, &synth[i_subfr], L_SUBFR, st->mem_syn, 1);
        }
        else
        {
