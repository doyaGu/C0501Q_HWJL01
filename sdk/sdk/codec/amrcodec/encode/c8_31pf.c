...
)
{
    ...
    //    sign_indx[i] = -1;
    //}
    Preset( -1, pos_indx, NB_TRACK_MR102);
    Preset( -1, sign_indx, NB_TRACK_MR102);
    ...
    for (i = 0; i < L_CODE; i++)
    {
       ...
       //y[i] = round (s);
       y[i] = (Word16)(L_add(s,(Word32)0x00008000L)>>16);
       ...
       Word16 pos_indxC) /* i : position and sign of 8 pulses (compressed) */
{
   ...
   ib = (Word16)((pos_indxB >> 1) * 5);

   //ic = extract_l(L_shr(L_mult(shr(pos_indxC, 1), 25), 1));
   ic = (Word16)((pos_indxC >> 1) * 25);
    ...
    Word16 indx[])      /* o : position and sign of 8 pulses (compressed) */
{
   ...
   indx[NB_TRACK_MR102+1]= compress10(pos_indx[2],pos_indx[6],pos_indx[5]);
    ...
    ib = (Word16)((pos_indx[7]>>1) * 5);

    //ib = add(shl(add(ia, ib), 5), 12);
    ib = ((ia + ib) << 5) + 12;

    //ic = shl(mult(ib, 1311), 2);
    //ic =  (Word16)(((Word16)(L_mult(ib, 1311)>>16)) << 2);
    ic = (Word16)(((ib * 1311)>>15)<<2);
