...
Word16 vad2 (Word16 * farray_ptr, vadState2 * st)
{
    ...
    else
    {
        ...
        if (sub(xt, st->tsnr) > 0)
        {
            ...
            st->tsnr = round16(L_add(L_mult(29491, st->tsnr), L_mult(3277, xt)));
        }
        /* else if (xt > 0.625*tsnr) */
        else if (sub(xt, mult(20480, st->tsnr)) > 0)
        {
            /* tsnr = 0.998*tsnr + 0.002*xt; */
            st->tsnr = round16(L_add(L_mult(32702, st->tsnr), L_mult(66, xt)));
        }
    }
    ...
    if (xt < 0)
    {
        ...
        tmp = round16(L_shl(L_mult(xt, xt), 7));
        st->negSNRvar = round16(L_add(L_mult(32440, st->negSNRvar), L_mult(328, tmp)));
    ...
    for (i = LO_CHAN; i <= HI_CHAN; i++)
    {
        ...
        st->ch_enrg_long_db[i] = round16(L_add(Ltmp1, Ltmp2));
    }
...
int vad2_init (vadState2 **state)
{
    ...
    *state = NULL;
    ...
    /* allocate memory */
    if ((s = (vadState2 *) SYS_Malloc(sizeof(vadState2))) == NULL){
