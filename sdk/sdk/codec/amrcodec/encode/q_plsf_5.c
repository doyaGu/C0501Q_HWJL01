    ...
    Word16 dico_size)       /* i : size of quantization codebook Q0  */
{
    ...
    for (i = 0; i < dico_size; i+=8)
    {
        ...
        temp = lsf_r2[0] - p_dico[i+2];
        temp = ( (wf2[0] * temp)>>15 );
        dist += (temp * temp);

        temp = lsf_r2[1] - p_dico[i+3];
        temp = ( (wf2[1] * temp)>>15 );
        dist += (temp * temp);
...
)
{
    ...
    for (i = 0; i < M; i++)
    {
        lsf_p[i] = mean_lsf[i] + (Word16)((st->past_rq[i] * LSP_PRED_FAC_MR122)>>15);
    ...
    indice[0] = Vq_subvec (&lsf_r1[0], &lsf_r2[0], dico1_lsf,
                           &wf1[0], &wf2[0], 128);     // DICO1_SIZE = 128

    indice[1] = Vq_subvec (&lsf_r1[2], &lsf_r2[2], dico2_lsf, // DICO1_SIZE = 256
                           &wf1[2], &wf2[2], 256);

    indice[2] = Vq_subvec_s (&lsf_r1[4], &lsf_r2[4], dico3_lsf, // DICO1_SIZE = 256
                             &wf1[4], &wf2[4], 256);

    indice[3] = Vq_subvec (&lsf_r1[6], &lsf_r2[6], dico4_lsf,  // DICO1_SIZE = 256
                           &wf1[6], &wf2[6], 256);

    indice[4] = Vq_subvec (&lsf_r1[8], &lsf_r2[8], dico5_lsf,  // DICO1_SIZE = 64
                           &wf1[8], &wf2[8], 64);
