...
#  endif
#else
static a52_state_t a52StateInfo;
static sample_t    a52SampleOut[256*12];
#endif

static sample_t *a52_samples(a52_state_t * state);
//void a52_dynrng(a52_state_t * state, level_t(*call) (level_t, void *), void *data);
static int a52_frame(a52_state_t * state, uint8_t * buf, int *flags, level_t * level, sample_t bias);
static int a52_block(a52_state_t * state);

static __inline int16_t convert (sample_t i)
{
#ifdef LIBA52_FIXED
    i >>= 15;
#else
...
static __inline void convert2s16_2 (sample_t * _f, int16_t * s16)
{
    int i;
...
a52_state_t *a52_init(void)
{
    ...
    #else
    state = (a52_state_t *) &a52StateInfo;
    state->samples = (sample_t *) a52SampleOut;
...
static int parse_exponents(a52_state_t * state, int expstr, int ngrps, uint8_t exponent, uint8_t * dest)
{
    ...
    while (ngrps--) {
        ...
        exponent += exp_1[exps];
        if (exponent > 24)
            return ERR_AC3_INVALID_EXPONENT;
...
static __inline int zero_snr_offsets(int nfchans, a52_state_t * state)
{
    int i;
...
static __inline int16_t dither_gen(a52_state_t * state)
{
    int16_t nstate;
...
static void coeff_get(a52_state_t * state, sample_t * coeff,
                      expbap_t * expbap, quantizer_set_t * quant, level_t level, int dither, int end)
{
    int i;
    uint8_t *exp;
    int8_t *bap;
...
static void coeff_get_coupling(a52_state_t * state, int nfchans,
                               level_t * coeff, sample_t(*samples)[256], quantizer_set_t * quant, uint8_t dithflag[5])
{
        ...
        if (zero_snr_offsets(nfchans, state)) {
            ...
            if (state->chincpl && (do_bit_alloc & 64))  /* cplinu */
                a52_bit_allocate(state, &state->cplba, state->cplstrtbnd,
                                 ...
                                 state->cplfleak << 8, state->cplsleak << 8, &state->cpl_expbap);
            for (i = 0; i < nfchans; i++)
                if (do_bit_alloc & (1 << i))
...
int a52_decode (a52_state_t * state, int * flags, uint8_t * inbuf,
                int * bytesLeft, uint8_t * outbuf)
{
    int i, k;
    int length;
    ...
    PRINTF("==============================\n");
    // Decode frame header
    length = a52_syncinfo (inbuf, flags, &sample_rate, &bit_rate);
    if (length < 0) return length;
    if (*bytesLeft < length) return ERR_AC3_INDATA_UNDERFLOW;

    PRINTF("Frame Length : %d\n", length);
    #if defined(__DEBUG__)
    switch(*flags & 0xf) {
        case A52_CHANNEL      : PRINTF("Frame Type   : A52_CHANNEL");      break;
        case A52_MONO         : PRINTF("Frame Type   : A52_MONO");         break;
        case A52_STEREO       : PRINTF("Frame Type   : A52_STEREO");       break;
        case A52_3F           : PRINTF("Frame Type   : A52_3F");           break;
        case A52_2F1R         : PRINTF("Frame Type   : A52_2F1R");         break;
        case A52_3F1R         : PRINTF("Frame Type   : A52_3F1R");         break;
        case A52_2F2R         : PRINTF("Frame Type   : A52_2F2R");         break;
        case A52_3F2R         : PRINTF("Frame Type   : A52_3F2R");         break;
        case A52_CHANNEL1     : PRINTF("Frame Type   : A52_CHANNEL1");     break;
        case A52_CHANNEL2     : PRINTF("Frame Type   : A52_CHANNEL2");     break;
        case A52_DOLBY        : PRINTF("Frame Type   : A52_DOLBY");        break;
        case A52_CHANNEL_MASK : PRINTF("Frame Type   : A52_CHANNEL_MASK"); break;
    }
    if (*flags & A52_LFE) PRINTF(", A52_LFE");
    if (*flags & A52_ADJUST_LEVEL) PRINTF(", A52_ADJUST_LEVEL");
    PRINTF("\n");
    #endif
    ...
    *flags = A52_STEREO | A52_ADJUST_LEVEL; // Force to stereo output
    ...
    // decode frame
    status = a52_frame (state, inbuf, flags, &level, bias);
    if (status < 0) return status;

    for (i = 0, k = 0; i < 6; i++, k += (256*4)) {
        status = a52_block(state);
        if (status < 0) return status;
        convert2s16_2(a52_samples (state), (int16_t *)&outbuf[k]);
    }
