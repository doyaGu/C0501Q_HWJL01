 ...
 *
 **************************************************************************************/
static void MidSideProc(int x[MAX_NCHAN][MAX_NSAMP], int nSamps)
{
    int i, xr, xl;
 ...
 **************************************************************************************/
static void IntensityProcMPEG1(int x[MAX_NCHAN][MAX_NSAMP], int nSamps, FrameHeader *fh, ScaleFactorInfoSub *sfis,
                        CriticalBandInfo *cbi, int midSideFlag, int mixFlag)
{
    int i=0, j=0, n=0, cb=0, w=0;
    int sampsLeft, isf, xl, xr;
    ...
    for (cb = cbStartL; cb < cbEndL && sampsLeft > 0; cb++) {
        ...
        for (j = 0; j < n && sampsLeft > 0; j++, i++) {
            xr = MULSHIFT(fr, x[0][i], 32-2);   x[1][i] = xr;
            xl = MULSHIFT(fl, x[0][i], 32-2);   x[0][i] = xl;
    ...
    for (cb = cbStartS; cb < cbEndS && sampsLeft >= 3; cb++) {
        ...
        for (j = 0; j < n && sampsLeft >= 3; j++, i+=3) {
            xr = MULSHIFT(frs[0], x[0][i+0], 32-2);     x[1][i+0] = xr;
            xl = MULSHIFT(fls[0], x[0][i+0], 32-2);     x[0][i+0] = xl;
            xr = MULSHIFT(frs[1], x[0][i+1], 32-2);     x[1][i+1] = xr;
            xl = MULSHIFT(fls[1], x[0][i+1], 32-2);     x[0][i+1] = xl;
            xr = MULSHIFT(frs[2], x[0][i+2], 32-2);     x[1][i+2] = xr;
            xl = MULSHIFT(fls[2], x[0][i+2], 32-2);     x[0][i+2] = xl;
 ...
 **************************************************************************************/
static void IntensityProcMPEG2(int x[MAX_NCHAN][MAX_NSAMP], int nSamps, FrameHeader *fh, ScaleFactorInfoSub *sfis,
                        CriticalBandInfo *cbi, ScaleFactorJS *sfjs, int midSideFlag, int mixFlag)
{
    int i, j, k, n, r, cb, w;
    int fl, fr, xl, xr;
    int sampsLeft;
 ...
 * Function:    Dequantize
 *
 * Description: decode stereo, reorder short blocks (two granule-worth)
 *
 ...
 *                (two granule-worth, all channels), format = Q26
 ...
 *           2. modified from int Dequantize(gr) in dequant.c
 **************************************************************************************/
    ...
    for (gr = 0; gr < mp3DecInfo.nGrans; gr++) {
        ...
        if (fh->modeExt >> 1) {
            if (fh->modeExt & 0x01) {
                ...
                nSamps = MAX(hi->nonZeroBound[gr][0], hi->nonZeroBound[gr][1]);
            }
            MidSideProc(hi->huffDecBuf[gr], nSamps);
        }
    }
    for (gr = 0; gr < mp3DecInfo.nGrans; gr++) {
        // do intensity stereo processing, if enabled
        if (fh->modeExt & 0x01) {
            nSamps = hi->nonZeroBound[gr][0];
            if (fh->ver == MPEG1) {
                IntensityProcMPEG1(hi->huffDecBuf[gr], nSamps, fh, &sfi->sfis[gr][1], di->cbi,
                    fh->modeExt >> 1, si->sis[gr][1].mixedBlock);
            } else {
                IntensityProcMPEG2(hi->huffDecBuf[gr], nSamps, fh, &sfi->sfis[gr][1], di->cbi, &sfi->sfjs[gr],
                    fh->modeExt >> 1, si->sis[gr][1].mixedBlock);
            }
        }
        ...
        if (fh->modeExt) {
            nSamps = MAX(hi->nonZeroBound[gr][0], hi->nonZeroBound[gr][1]);
            hi->nonZeroBound[gr][0] = nSamps;
            hi->nonZeroBound[gr][1] = nSamps;
        }
    }
    // output format Q(DQ_FRACBITS_OUT)
    // yiwei, by experiment, the output seems always S0.26 format
}
