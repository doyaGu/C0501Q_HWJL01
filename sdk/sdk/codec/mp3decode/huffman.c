...
static int DecodeHuffmanPairs(int *xy, int nVals, int tabIdx, int bitsLeft, unsigned char *buf, int bitOffset)
{
    ...
    if ((nVals & 0x01) || (tabIdx  >= HUFF_PAIRTABS) ||
        (tabIdx < 0)   || (tabType == invalidTab)) {
        return -1;
    }
...
static void udiv5bit(unsigned int dividend, unsigned int divisor, unsigned int *quotient)
{
    ...
    numzeros = 0;
    while (!(divisor & (1<<(NBITS5-1)) )) { // assume dividend, divisor is 12bit unsigned integer
        numzeros++;
        divisor <<= 1;
    }
    q=0;
...
int DecodeHuffman(unsigned char *buf, int *bitOffset, int huffBlockBits, int gr, int ch)
{
    ...
    if (sis->winSwitchFlag && sis->blockType == 2) {
        if (sis->mixedBlock == 0) {
            // yiwei, because region0Count has 4 bits, we use 5 bit division
            #if defined(USER_DEFINE_DIV)
            int div3;
            udiv5bit(sis->region0Count + 1, 3, &div3);
            r1Start = fh->sfBand->s[div3] * 3;
            #else
    ...
    hi->nonZeroBound[gr][ch] = rEnd[3];

    /* decode Huffman pairs (rEnd[i] are always even numbers) */
    bitsLeft = huffBlockBits;
    for (i = 0; i < 3; i++) {
        bitsUsed = DecodeHuffmanPairs(hi->huffDecBuf[gr][ch] + rEnd[i], rEnd[i+1] - rEnd[i], sis->tableSelect[i], bitsLeft, buf, *bitOffset);
    ...
    hi->nonZeroBound[gr][ch] += DecodeHuffmanQuads(hi->huffDecBuf[gr][ch] + rEnd[3], MAX_NSAMP - rEnd[3], sis->count1TableSelect, bitsLeft, buf, *bitOffset);

    ASSERT(hi->nonZeroBound[gr][ch] <= MAX_NSAMP);
    if (hi->nonZeroBound[gr][ch] > MAX_NSAMP)
        return -1;

    for (i = hi->nonZeroBound[gr][ch]; i < MAX_NSAMP; i++)
        hi->huffDecBuf[gr][ch][i] = 0;
    ...
    *bitOffset = (bitsLeft + *bitOffset) & 0x07;
    ...
    {
        fprintf(dbgST_DecodeHuffman, "nFrames=%d, gr=%d, ch=%d\n", nFrames, gr, ch);
        fprintf(dbgST_DecodeHuffman, "huffDecBuf[576]=\n");
        for(i=0; i<MAX_NSAMP; i++){
            fprintf(dbgST_DecodeHuffman, "%d,", hi->huffDecBuf[gr][ch][i]);
        }
        fprintf(dbgST_DecodeHuffman, "\nnonZeroBound[ch=%d]=\n", ch);
        fprintf(dbgST_DecodeHuffman, "%d,", hi->nonZeroBound[gr][ch]);
        fprintf(dbgST_DecodeHuffman, "\n\n");
    }
    #endif
