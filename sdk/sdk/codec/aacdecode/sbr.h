...
# define FFT32C                         STATNAME(f101)
# define CalcFreqTables                 STATNAME(f102)
# define AdjustHighFreq                 STATNAME(f103)
# define GenerateHighFreq               STATNAME(f104)
# define DecodeSBREnvelope              STATNAME(f105)
# define DecodeSBRNoise                 STATNAME(f106)
# define UncoupleSBREnvelope            STATNAME(f107)
# define UncoupleSBRNoise               STATNAME(f108)
# define InvRNormalized                 STATNAME(f109)
# define RatioPowInv                    STATNAME(f110)
# define SqrtFix                        STATNAME(f111)
# define QMFAnalysis                    STATNAME(f112)
# define QMFSynthesis                   STATNAME(f113)
# define GetSampRateIdx                 STATNAME(f114)
# define UnpackSBRHeader                STATNAME(f115)
# define UnpackSBRSingleChannel         STATNAME(f116)
# define UnpackSBRChannelPair           STATNAME(f117)
...
typedef struct{
    int     bufTemp[8][64][2]; // 8*64*2*4 = 4096
} BufTemp;
...
typedef struct _PSInfoSBR {
    ...
    // same == faad's qmfs->v_index
    int                   delayIdxQMFS[AAC_MAX_NCHANS];
    // same == faad's qmfs_info
    ...
    BufTemp        *XbufTmp2;
#ifdef PARSING_HE_AAC_V2
    int                   delayQMFSPS[AAC_MAX_NCHANS][DELAY_SAMPS_QMFS];
    unsigned char nExtensionID;
    ps_info *ps;
