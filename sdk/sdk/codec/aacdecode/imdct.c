 ...
 **************************************************************************************/
static void DecWindowOverlap(int *buf0, int *over0, PCM_WORD *pcm0,
                             int nChans, int winTypeCurr, int winTypePrev)
{
    int in, w0, w1, f0, f1;
    int *buf1, *over1;
    PCM_WORD *pcm1;
    ...
    if (winTypeCurr == winTypePrev) {
        ...
        do {
            ...
            #if defined(REVERBERATION) || defined(DRCTL)
            *pcm0 = (in - f0 + RND_VAL) >> FBITS_OUT_IMDCT;
            pcm0 ++;
            in = *over1;
            *pcm1 = (in + f1 + RND_VAL) >> FBITS_OUT_IMDCT;
            pcm1 --;
            #else
 ...
 *              asm code with free pointer updates, better load scheduling
 **************************************************************************************/
static void DecWindowOverlapLongStart(int *buf0, int *over0, PCM_WORD *pcm0,
                                      int nChans, int winTypeCurr, int winTypePrev)
{
 ...
 *              asm code with free pointer updates, better load scheduling
 **************************************************************************************/
static void DecWindowOverlapLongStop(int *buf0, int *over0, PCM_WORD *pcm0,
                                     int nChans, int winTypeCurr, int winTypePrev)
{
    int i, in, w0, w1, f0, f1;
    int *buf1, *over1;
    ...
    do {
        ...
        #if defined(REVERBERATION) || defined(DRCTL)
        *pcm0 = (in + RND_VAL) >> FBITS_OUT_IMDCT;
        pcm0 ++;
        in = *over1;
        *pcm1 = (in + f1 + RND_VAL) >> FBITS_OUT_IMDCT;
        pcm1 --;
 ...
 *              asm code with free pointer updates, better load scheduling
 **************************************************************************************/
static void DecWindowOverlapShort(int *buf0, int *over0, PCM_WORD *pcm0,
                                  int nChans, int winTypeCurr, int winTypePrev)
{
    int i, in, w0, w1, f0, f1;
    int *buf1, *over1;
    ...
    do {
        ...
        f1 = *over0++;
        #if defined(REVERBERATION) || defined(DRCTL)
        *pcm0++ = (f0 + RND_VAL) >> FBITS_OUT_IMDCT;
        *pcm0++ = (f1 + RND_VAL) >> FBITS_OUT_IMDCT;
        #else
 ...
 **************************************************************************************/
int IMDCT(AACDecInfo *aacDecInfo, int ch, int chOut, PCM_WORD *outbuf)
{
    int i;
    int nTemp;
    PSInfoBase *psi;
    ICSInfo *icsInfo;
#ifdef ENABLE_DOWNMIX_CHANNLES
    short nLeftChannel;
    short nRightChannel;
#endif
    ...
    if(isEQ()){
        or32_invalidate_cache(&eqinfo, sizeof(eqinfo));
        if(eqinfo.updEQ != 0){
            if(eqinfo.endianEQ != 1){
                unsigned short tmp;
                for (i=0; i<sizeof(eqinfo.dbtab)/sizeof(short); i++) {
                    tmp = eqinfo.dbtab[i];
                    eqinfo.dbtab[i] = (tmp << 8) + (tmp >> 8);
                    tmp = eqinfo.bandcenter[i];
                    eqinfo.bandcenter[i] = (tmp << 8) + (tmp >> 8);
                }
            }
            eqinfo.endianEQ = 1;
            eqinfo.updEQ    = 0;
            computeEQGain(aacDecInfo->sampRate);
        }
        // do equalize here;
        if (icsInfo->winSequence == 2) {
            /* 8 short blocks */
            for (i = 0; i < 8; i++)
                equalize(psi->coef[ch] + i*128, 0);
        } else {
            /* 1 long block */
            equalize(psi->coef[ch], 1);
        }
    }
    ...
    if (icsInfo->winSequence == 2) {
       ...
       #ifndef ENABLE_DOWNMIX_CHANNELS
           // just keep left,right channels to outputs
           if(aacDecInfo->nChans>2 && (chOut>=2 && chOut<= AAC_MAX_NCHANS) )
           {
              nTemp = 2;
              return 0;
           }
           else if(aacDecInfo->nChans>2 && (chOut<=1) )
           {
              nTemp = 2;
           }
           else if(aacDecInfo->nChans == 1)
           {
              nTemp = aacDecInfo->nChans;
           }
       #else 
           nTemp = aacDecInfo->nChans;
       #endif
    ...
    if (!aacDecInfo->sbrEnabled) {
        for (i = 0; i < AAC_MAX_NSAMPS; i++) {
            #if defined(REVERBERATION) || defined(DRCTL)
            *outbuf = (psi->sbrWorkBuf[ch][i] + RND_VAL) >> FBITS_OUT_IMDCT;
            outbuf++;
            #else
    ...
    for(i=0;i<MAX_NCHANS_ELEM;i++)
    {
        aacDecInfo->rawSampleBuf[i] = psi->sbrWorkBuf[i];
    }
    aacDecInfo->rawSampleBytes = sizeof(int);
    aacDecInfo->rawSampleFBits = FBITS_OUT_IMDCT;
#else // !defined(AAC_ENABLE_SBR)
    /* window, overlap-add, round to PCM - optimized for each window sequence */
    if (icsInfo->winSequence == 0)
        DecWindowOverlap(psi->coef[ch], psi->overlap[chOut], outbuf, nTemp, icsInfo->winShape, psi->prevWinShape[chOut]);
    else if (icsInfo->winSequence == 1)
        DecWindowOverlapLongStart(psi->coef[ch], psi->overlap[chOut], outbuf, nTemp, icsInfo->winShape, psi->prevWinShape[chOut]);
    else if (icsInfo->winSequence == 2)
        DecWindowOverlapShort(psi->coef[ch], psi->overlap[chOut], outbuf, nTemp, icsInfo->winShape, psi->prevWinShape[chOut]);
    else if (icsInfo->winSequence == 3)
        DecWindowOverlapLongStop(psi->coef[ch], psi->overlap[chOut], outbuf, nTemp, icsInfo->winShape, psi->prevWinShape[chOut]);
