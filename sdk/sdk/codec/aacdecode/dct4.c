...
static void PreMultiply(int tabidx, int *zbuf1)
{
    ...
    for (i = nmdct >> 2; i != 0; i--) {
         ...
         */
        *(zbuf1+0) = MULSHIFT32_ADD(cos2a, ar1, sin2a, ai1);  /* cos*ar1 + sin*ai1 */
        *(zbuf1+1) = MULSHIFT32_SUB(cos2a, ai1, sin2a, ar1);  /* cos*ai1 - sin*ar1 */

        *(zbuf2-0) = MULSHIFT32_SUB(cos2b, ai2, sin2b, ar2);  /* cos*ai2 - sin*ar2 */
...
static void PostMultiplyRescale(int tabidx, int *fft1, int es)
{
    ...
    for (i = nmdct >> 2; i != 0; i--) {
        ...
        z = MULSHIFT32_SUB(sin2, ar1, cos2, ai1);	//sin*ar1 - cos*ai1
        CLIP_2N_SHIFT(z, es);
        *fft2 = z;
        z = MULSHIFT32_ADD(sin2, ai1, cos2, ar1);	//sin*ai1 + cos*ar1
        CLIP_2N_SHIFT(z, es);
        *fft1 = z;
        ...
        z = MULSHIFT32_ADD(cos2, ai2, sin2, ar2);	//cos*ai2 + sin*ar2
        CLIP_2N_SHIFT(z, es);
        *(fft2-1) = z;
        z = MULSHIFT32_SUB(cos2, ar2, sin2, ai2);	//cos*ar2 - sin*ai2
        CLIP_2N_SHIFT(z, es);
        *(fft1+1) = z;
		fft2 -= 2;
		fft1 += 2;
