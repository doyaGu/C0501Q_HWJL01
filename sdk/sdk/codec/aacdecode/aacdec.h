/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: aacdec.h,v 1.4 2005/06/20 17:30:44 ehodge Exp $
 *
...
enum {
    ...
    ERR_AAC_LATM_PARSING_ERROR            = -23,
    ERR_AAC_SBR_FREQ                      = -24,
    ERR_AAC_HUFFMAN_ERROR                 = -25,
    ERR_AAC_SBR_QMF_DATA_ERROR      = -26,    
    ERR_AAC_SBR_GRID_DATA_ERROR     = -27,    
    ERR_UNKNOWN                           = -9999
};
...
typedef struct _AACMultiChannels 
{   // support 5 and 5.1 channels
    short nCenter;
    short nLeft;
    short nRight;
    short nLeftRear;
...
# define AACInitDecoder                 STATNAME(AACInitDecoder)
# define AACDecode                      STATNAME(AACDecode)
# define AACFindSyncWord                STATNAME(AACFindSyncWord)
# define AACGetLastFrameInfo            STATNAME(AACGetLastFrameInfo)
# define AACSetRawBlockParams           STATNAME(AACSetRawBlockParams)
# define AACFlushCodec                  STATNAME(AACFlushCodec)
# define AACInitTrigtabsFloat           STATNAME(AACInitTrigtabsFloat)
# define AACFreeTrigtabsFloat           STATNAME(AACFreeTrigtabsFloat)
#else // defined(__FREERTOS__)
# define AACInitDecoder                 STATNAME(f401)
# define AACDecode                      STATNAME(f402)
# define AACFindSyncWord                STATNAME(f403)
# define AACGetLastFrameInfo            STATNAME(f404)
# define AACSetRawBlockParams           STATNAME(f405)
# define AACFlushCodec                  STATNAME(f406)
# define AACInitTrigtabsFloat           STATNAME(f407)
# define AACFreeTrigtabsFloat           STATNAME(f408)
#endif // !defined(__FREERTOS__)
#endif // defined(USE_STATNAME)

/* public C API */
/*MMP_INLINE*/ HAACDecoder AACInitDecoder(void);
/*MMP_INLINE*/ int  AACDecode(HAACDecoder hAACDecoder, unsigned char **inbuf, int *bytesLeft, short *outbuf);

/*MMP_INLINE*/ int  AACFindSyncWord(unsigned char *buf, int nBytes);
/*MMP_INLINE*/ void AACGetLastFrameInfo(HAACDecoder hAACDecoder, AACFrameInfo *aacFrameInfo);
/*MMP_INLINE*/ int  AACSetRawBlockParams(HAACDecoder hAACDecoder, int copyLast, AACFrameInfo *aacFrameInfo);
/*MMP_INLINE*/ int  AACFlushCodec(HAACDecoder hAACDecoder);
/*MMP_INLINE*/ int ADTSSyncInfo( unsigned char *buf);

#ifdef HELIX_CONFIG_AAC_GENERATE_TRIGTABS_FLOAT
/*MMP_INLINE*/ int  AACInitTrigtabsFloat(void);
/*MMP_INLINE*/ void AACFreeTrigtabsFloat(void);
#endif
