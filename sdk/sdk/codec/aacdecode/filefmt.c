...
int UnpackADTSHeader(AACDecInfo *aacDecInfo, unsigned char **buf, int *bitOffset, int *bitsAvail)
{
    ...
    PRINTF("=========== ADTS Header ===========\n");
    PRINTF("Sync Word        (12) : 0x%03x\n", 0xfff);
    PRINTF("ID               ( 1) : 0x%01x\n", fhADTS->id);
    PRINTF("Layer            ( 2) : 0x%01x\n", fhADTS->layer);
    PRINTF("protectBit       ( 1) : 0x%01x\n", fhADTS->protectBit);
    PRINTF("profile          ( 2) : 0x%01x\n", fhADTS->profile);
    PRINTF("sampRateIdx      ( 4) : 0x%01x\n", fhADTS->sampRateIdx);
    PRINTF("privateBit       ( 1) : 0x%01x\n", fhADTS->privateBit);
    PRINTF("channelConfig    ( 3) : 0x%01x\n", fhADTS->channelConfig);
    PRINTF("origCopy         ( 1) : 0x%01x\n", fhADTS->origCopy);
    PRINTF("home             ( 1) : 0x%01x\n", fhADTS->home);
    PRINTF("copyBit          ( 1) : 0x%01x\n", fhADTS->copyBit);
    PRINTF("copyStart        ( 1) : 0x%01x\n", fhADTS->copyStart);
    PRINTF("frameLength      (13) : 0x%04x\n", fhADTS->frameLength);
    PRINTF("bufferFull       (11) : 0x%03x\n", fhADTS->bufferFull);
    PRINTF("numRawDataBlocks ( 2) : 0x%01x\n", fhADTS->numRawDataBlocks-1);

// fhADTS->layer indicates which layer is used. Set to '00' on normal case.
// I add the extension '01' to add the PTS information on it.
// See ISO/IEC 11172-3, subclause 2.4.2.3.
#if defined(ADTS_EXTENSION_FOR_PTS)
    if (fhADTS->layer == 1) {
        fhADTS->PTS =          GetBits(&bsi, 8) +
                               (GetBits(&bsi, 8) << 8)  +
                               (GetBits(&bsi, 8) << 16) +
                               (GetBits(&bsi, 8) << 24) ;
        PRINTF("PTS              (32) : 0x%04x\n", fhADTS->PTS);

        // The original frame lenth dose not include PTS length.
        // fhADTS->frameLength += 4;
    }
    ...
    fhADTS->profile = 1;        // Dont care the profile!! Modify by Kuoping
/*
    if (fhADTS->id ==0)
        fhADTS->profile -= 1;   // Mpeg2Profile = Mpeg4ObjType  - 1;
*/

#if defined(ADTS_EXTENSION_FOR_PTS)
    if ((fhADTS->layer != 0 && fhADTS->layer != 1) ||
    ...
    aacDecInfo->nFrameLength = fhADTS->frameLength;
#if defined(ADTS_EXTENSION_FOR_PTS)
    aacDecInfo->PTS = (fhADTS->layer == 1) ? fhADTS->PTS : -1;
#endif // defined(ADTS_EXTENSION_FOR_PTS)

    if (*bitsAvail/8 < fhADTS->frameLength)
...
#if defined(ADTS_EXTENSION_FOR_PTS)
    aacDecInfo->PTS = -1; // NO PTS in ADIF format
#endif // defined(ADTS_EXTENSION_FOR_PTS)
