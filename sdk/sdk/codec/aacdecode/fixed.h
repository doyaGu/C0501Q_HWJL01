...
#define MUL_C1(A,B) (real_t)(((int64_t)(A)*(int64_t)(B)+(1 << (COEF_BITS-1))) >> COEF_BITS)
    ...
    real_t x1, real_t x2, real_t c1, real_t c2)
{
    ...
    *y2 = yt2 << (FRAC_SIZE-FRAC_BITS);
    ...
    static INLINE real_t MUL_F(real_t A, real_t B)
    {
        ...
        asm volatile("l.macrc %0,%1" : "=r"(nResult): "i"(FRAC_BITS));       
        return nResult;
    }

    static INLINE real_t MUL_SUB_F(real_t A, real_t B)
    {
        int nResult;
        asm volatile("l.msb %0, %1" : : "r"(A), "r"(B));
        asm volatile("l.macrc %0,%1" : "=r"(nResult): "i"(FRAC_BITS));       
        return nResult;
    }
  ...
  static INLINE real_t MUL_R(real_t A, real_t B)
  {
        int nResult;
        asm volatile("l.mac %0, %1" : : "r"(A), "r"(B));
        asm volatile("l.macrc %0,%1" : "=r"(nResult): "i"(REAL_BITS) );        
        return nResult;
  }
  ...
  static INLINE real_t MUL_C(real_t A, real_t B)
  {
        int nResult;
        asm volatile("l.mac %0, %1" : : "r"(A), "r"(B));
        asm volatile("l.macrc %0,%1" : "=r"(nResult): "i"(COEF_BITS) );        
        return nResult;
  }

  static INLINE real_t MUL_SUB_C(real_t A, real_t B)
  {
        int nResult;
        asm volatile("l.msb %0, %1" : : "r"(A), "r"(B));
        asm volatile("l.macrc %0,%1" : "=r"(nResult): "i"(COEF_BITS) );        
    ...
    static __inline int MUL_C_ADD(int a, int b, int c, int d)
    {
        int nResult;
        asm volatile ("l.mac %0, %1" : : "r"(a), "r"(b));
    ...
    static __inline int MUL_C_SUB(int a, int b, int c, int d)
    {
        int nResult;
        asm volatile ("l.mac %0, %1" : : "r"(a), "r"(b));
  ...
  static INLINE real_t _MulHigh(real_t A, real_t B)
  {
        ...
        asm volatile("l.mac %0, %1" : : "r"(A), "r"(B));
        asm volatile("l.macrc %0,%1" : "=r"(nResult): "i"(FRAC_SIZE) );        
        return nResult;
  }
        ...
        real_t x1, real_t x2, real_t c1, real_t c2)
    {
        ...
        //*y1 = (_MulHigh(x1, c1) + _MulHigh(x2, c2));
        //*y2 = (_MulHigh(x2, c1) - _MulHigh(x1, c2));

        asm volatile("l.mac %0, %1" : : "r"(x1), "r"(c1));
        asm volatile("l.mac %0, %1" : : "r"(x2), "r"(c2));        
        asm volatile("l.macrc %0,%1" : "=r"(*y1): "i"(FRAC_SIZE));        

        asm volatile("l.mac %0, %1" : : "r"(x2), "r"(c1));
        asm volatile("l.msb %0, %1" : : "r"(x1), "r"(c2));        
        asm volatile("l.macrc %0,%1" : "=r"(*y2): "i"(FRAC_SIZE));               
    }

    static __inline int MUL_R_ADD(int a, int b, int c, int d)
    {
        int nResult;
        asm volatile ("l.mac %0, %1" : : "r"(a), "r"(b));
        asm volatile ("l.mac %0, %1" : : "r"(c), "r"(d));
        asm volatile("l.macrc %0,%1" : "=r"(nResult): "i"(1));       
        
        return nResult;
    }

    static __inline int MUL_F_ADD(int a, int b, int c, int d)
    {
        int nResult;
        asm volatile ("l.mac %0, %1" : : "r"(a), "r"(b));
        asm volatile ("l.mac %0, %1" : : "r"(c), "r"(d));
    ...
    static __inline int MUL_F_SUB(int a, int b, int c, int d)
    {
        int nResult;
        asm volatile ("l.mac %0, %1" : : "r"(a), "r"(b));
        asm volatile ("l.msb %0, %1" : : "r"(c), "r"(d));
    ...
    static __inline int MUL_F_ADD_6(int a, int b, int c, int d, int e, int f)
    {
        int nResult;
        asm volatile ("l.mac %0, %1" : : "r"(a), "r"(b));
        asm volatile ("l.mac %0, %1" : : "r"(c), "r"(d));
        asm volatile ("l.mac %0, %1" : : "r"(e), "r"(f));        
        asm volatile("l.macrc %0,%1" : "=r"(nResult): "i"(FRAC_BITS));       
        
        return nResult;
    }

    static __inline int MUL_F_ADD_8(int a, int b, int c, int d,int e, int f,int g, int h)
    {
        int nResult;
        asm volatile ("l.mac %0, %1" : : "r"(a), "r"(b));
        asm volatile ("l.mac %0, %1" : : "r"(c), "r"(d));
        asm volatile ("l.mac %0, %1" : : "r"(e), "r"(f)); 
        asm volatile ("l.mac %0, %1" : : "r"(g), "r"(h));                
        asm volatile("l.macrc %0,%1" : "=r"(nResult): "i"(FRAC_BITS));       
