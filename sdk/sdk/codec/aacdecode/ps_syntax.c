...
static uint16_t ps_extension(ps_info *ps, BitStreamInfo *ld,
                             const uint8_t ps_extension_id,
                             const uint16_t num_bits_left);
static void huff_data(BitStreamInfo *ld, const uint8_t dt, const uint8_t nr_par,
                      ps_huff_tab t_huff, ps_huff_tab f_huff, int8_t *par);
static int8_t ps_huff_dec(BitStreamInfo *ld, ps_huff_tab t_huff);


uint16_t ps_data(ps_info *ps, BitStreamInfo *bsi, uint8_t *header)
{
    uint8_t tmp, n;
    uint16_t bits = 0;
    bits = (bsi->nBytes*8)+bsi->cachedBits;
    *header = 0;
    ...
    if (GetBits(bsi, 1))
    {
        *header = 1;
        ...
        ps->enable_iid = (uint8_t)GetBits(bsi, 1);

        if (ps->enable_iid)
        {
            ps->iid_mode = (uint8_t)GetBits(bsi, 3);
        ...
        ps->enable_icc = (uint8_t)GetBits(bsi,1);

        if (ps->enable_icc)
        {
            ps->icc_mode = (uint8_t)GetBits(bsi,3);
        ...
        ps->enable_ext = (uint8_t)GetBits(bsi,1);
    }
    ...
    ps->frame_class = (uint8_t)GetBits(bsi,1);
    tmp = (uint8_t)GetBits(bsi,2);
    ...
    if (ps->frame_class)
    {
        ...
        {
            ps->border_position[n] = (uint8_t)GetBits(bsi,5);
        }
    }
    ...
    if (ps->enable_iid)
    {
        ...
        {
            ps->iid_dt[n] = (uint8_t)GetBits(bsi,1);

            /* iid_data */
            if (ps->iid_mode < 3)
            {
                huff_data(bsi, ps->iid_dt[n], ps->nr_iid_par, t_huff_iid_def,f_huff_iid_def, ps->iid_index[n]);
            } else {
                huff_data(bsi, ps->iid_dt[n], ps->nr_iid_par, t_huff_iid_fine,f_huff_iid_fine, ps->iid_index[n]);
            }
        }
    }
    ...
    if (ps->enable_icc)
    {
        for (n = 0; n < ps->num_env; n++)
        {
            ps->icc_dt[n] = (uint8_t)GetBits(bsi,1);
            /* icc_data */
            huff_data(bsi, ps->icc_dt[n], ps->nr_icc_par, t_huff_icc,f_huff_icc, ps->icc_index[n]);
        }
    }
    ...
    if (ps->enable_ext)
    {
        ...
        uint16_t cnt = (uint16_t)GetBits(bsi,4);

        if (cnt == 15)
        {
            cnt += (uint16_t)GetBits(bsi,8);
        }
        //num_bits_left = 8 * cnt;
        num_bits_left = cnt<<3;
        while (num_bits_left > 7)
        {
            uint8_t ps_extension_id = (uint8_t)GetBits(bsi,2);
            num_bits_left -= 2;
            num_bits_left -= ps_extension(ps, bsi, ps_extension_id, num_bits_left);
        }
        GetBits(bsi,num_bits_left);
    }

    //bits = bits - bsi->nBytes*8 - bsi->cachedBits;
    bits = bits - (bsi->nBytes<<3) - bsi->cachedBits;
    ps->ps_data_available = 1;

    return bits;
}

static uint16_t ps_extension(ps_info *ps, BitStreamInfo *bsi,
                             const uint8_t ps_extension_id,
                             const uint16_t num_bits_left)
{
    uint8_t n;
    ...
    {
        ps->enable_ipdopd = (uint8_t)GetBits(bsi,1);

        if (ps->enable_ipdopd)
        {
            for (n = 0; n < ps->num_env; n++)
            {
                ps->ipd_dt[n] = (uint8_t)GetBits(bsi,1);                
                /* ipd_data */
                huff_data(bsi, ps->ipd_dt[n], ps->nr_ipdopd_par, t_huff_ipd,f_huff_ipd, ps->ipd_index[n]);
                ps->opd_dt[n] = (uint8_t)GetBits(bsi,1);

                /* opd_data */
                huff_data(bsi, ps->opd_dt[n], ps->nr_ipdopd_par, t_huff_opd,f_huff_opd, ps->opd_index[n]);
            }
        }
        GetBits(bsi,1);
    }
...
static void huff_data(BitStreamInfo *bsi, const uint8_t dt, const uint8_t nr_par,
                      ps_huff_tab t_huff, ps_huff_tab f_huff, int8_t *par)
{
    uint8_t n;
    ...
    if (dt)
    {
        ...
        for (n = 0; n < nr_par; n++)
        {
            par[n] = ps_huff_dec(bsi, t_huff);
        }
    } else {
        /* coded in frequency direction */
        par[0] = ps_huff_dec(bsi, f_huff);

        for (n = 1; n < nr_par; n++)
        {
            par[n] = ps_huff_dec(bsi, f_huff);
        }
    }
}

/* binary search huffman decoding */
static int8_t ps_huff_dec(BitStreamInfo *bsi, ps_huff_tab t_huff)
{
    uint8_t bit;
    int16_t index = 0;
