...
int DecodeHuffmanScalar(const signed short *huffTab, const HuffInfo *huffTabInfo, unsigned int bitBuf, signed int *val)
{
    ...
    do 
    {
        ...
        if (nTimeout<0)
        {
           printf("[AAC] huffman DecodeHuffmanScalar timeout %d \n",nTimeout);
           return ERR_UNKNOWN;
        }
    } while (t >= count);
...
int DecodeSpectrumLong(PSInfoBase *psi, BitStreamInfo *bsi, int ch)
{
    ...
    {
        printf("[AAC] DecodeSpectrumLong sampRateIdx err %d  \n",psi->sampRateIdx);
        return ERR_AAC_HUFFMAN_ERROR;
    }
...
int DecodeSpectrumShort(PSInfoBase *psi, BitStreamInfo *bsi, int ch)
{
    ...
    if (psi->sampRateIdx>NUM_SAMPLE_RATES || psi->sampRateIdx< 0 )
    {
        printf("[AAC] DecodeSpectrumShort sampRateIdx err %d  \n",psi->sampRateIdx);
        return ERR_AAC_HUFFMAN_ERROR;
    }
