...
static int FilterRegion(int size, int dir, int order, int *audioCoef, int *a, int *hist)
{
    ...
    do {
        /* sum64 = a0*y[n] = 1.0*y[n] */
        y = *audioCoef;
        asm volatile("l.mtspr r0, %0, 0x2801" : : "r"(y << FBITS_LPC_COEFS));
        asm volatile("l.mtspr r0, %0, 0x2802" : : "r"(y >> (32 - FBITS_LPC_COEFS)));

        /* sum64 += (a1*y[n-1] + a2*y[n-2] + ... + a[order-1]*y[n-(order-1)]) */
        for (j = order - 1; j > 0; j--) {
            asm ("l.mac %0, %1" : : "r"(hist[j]), "r"(a[j]));
            hist[j] = hist[j-1];
        }
        asm ("l.mac %0, %1" : : "r"(hist[0]), "r"(a[0]));

        asm volatile("l.mfspr %0, r0, 0x2802" :  "=r"(hi32));
        asm volatile("l.macrc %0, %c1" :"=r"(y) : "n"(FBITS_LPC_COEFS));
