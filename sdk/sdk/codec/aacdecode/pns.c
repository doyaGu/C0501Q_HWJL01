...
int PNS(AACDecInfo *aacDecInfo, int ch)
{
	...
	maxSFB = icsInfo->maxSFB;
	pnsLastVal = psi->pnsLastVal;
    checkCorr = (aacDecInfo->currBlockID == AAC_ID_CPE && psi->commonWin == 1 ? 1 : 0);

    gbMask = 0;
    for (gp = 0; gp < icsInfo->numWinGroup; gp++) {
        for (win = 0; win < icsInfo->winGroupLen[gp]; win++) {
            msMaskPtr = psi->msMaskBits + ((gp*maxSFB) >> 3);
            msMaskOffset = ((gp*maxSFB) & 0x07);
            ...
            for (sfb = 0; sfb < maxSFB; sfb++) {
                ...
                if (sfbCodeBook[sfb] == 13) {
                    if (ch == 0) {
                         ...
                         */
                        GenerateNoiseVector(coef, &pnsLastVal, width);
                        if (checkCorr && psi->sfbCodeBook[1][gp*maxSFB + sfb] == 13)
                            CopyNoiseVector(coef, psi->coef[1] + (coef - psi->coef[0]), width);
                    } else {
                        /* generate new vector if no correlation between channels */
                        genNew = 1;
                        if (checkCorr && psi->sfbCodeBook[0][gp*maxSFB + sfb] == 13) {
                    ...
                    gbMask |= ScaleNoiseVector(coef, width, psi->scaleFactors[ch][gp*maxSFB + sfb]);
                }
                coef += width;
            ...
            coef += (nSamps - sfbTab[maxSFB]);
        }
        sfbCodeBook += maxSFB;
        scaleFactors += maxSFB;
    }
	psi->pnsLastVal = pnsLastVal;
