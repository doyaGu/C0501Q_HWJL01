                        ...
                        int nMaster, int k0, int kStart, int numQMFBands, int sampRateIdx)
{
    ...
    do 
    {
        ...
        do 
        {
            ...
            if(j<=0)
            {
                printf("[AAC] sbr freq BuildPatches j %d \n",j);
                return 0;                            
            }
        } while (sb > k0 - 1 + msb - oddFlag);

        if(numPatches<0 || numPatches>MAX_NUM_PATCHES)
        {
            printf("[AAC] sbr freq BuildPatches numPatches %d \n",numPatches);
        }
        if (!patchNumSubbands)
        {
            printf("[AAC] sbrfreq patchNumSubbands[numPatches] null \n");
            return 0;            
        }        
        if (!patchStartSubband)
        {
            printf("[AAC] sbrfreq patchStartSubband[numPatches] null \n");
            return 0;            
        }        
    ...
    } while (sb != (kStart + numQMFBands) && numPatches <= MAX_NUM_PATCHES && nCount <500);
                           ...
                           int nLow, int kStart, int limiterBands, int numPatches)
{
    ...
    if (numPatches > MAX_NUM_PATCHES || numPatches<0)
    {
          return ERR_AAC_SBR_FREQ;
    }
    if (limiterBands > 3 || limiterBands<0)
    {
          return ERR_AAC_SBR_FREQ;
    }
...
int CalcFreqTables(SBRHeader *sbrHdr, SBRFreq *sbrFreq, int sampRateIdx)
{
    ...
    if (sbrFreq->nLimiter == ERR_AAC_SBR_FREQ)
    {
        return ERR_AAC_SBR_FREQ;
    }
