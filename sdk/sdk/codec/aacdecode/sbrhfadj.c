 ...
 **************************************************************************************/
#if !defined(OR32_ASM)
static int EstimateEnvelope(PSInfoSBR *psi, SBRHeader *sbrHdr, SBRGrid *sbrGrid, SBRFreq *sbrFreq, int env)
{
    ...
    int nResult=ERR_AAC_NONE;
	int kStart = sbrFreq->kStart;
    int nTemp;
    ...
    if (sbrHdr->interpFreq)
    {
        for (m = 0; m < sbrFreq->numQMFBands; m++) 
        {
            ...
            nTemp = iStart;
            if (iStart<8)
                XBuf = psi->XbufTmp1->bufTemp[iStart][kStart + m];
            else if (iStart < 16)
                XBuf = psi->XbufTmp2->bufTemp[iStart-8][kStart + m];
            else
            XBuf = psi->XBuf[iStart][kStart + m];
#else
            XBuf = psi->XBuf[iStart][kStart + m];
            nTemp = iStart;
#endif
            for (i = iStart; i < iEnd; i++) 
            {
                ...
                XBuf -= 1;
                nTemp +=1;
                XBuf = psi->XBuf[nTemp][kStart + m];    
    #else
                XBuf += (2*64 - 1);
    #endif
    #ifdef AAC_ENABLE_INTERNAL_SD
                if (nTemp<8)
                    XBuf = psi->XbufTmp1->bufTemp[nTemp][kStart + m];
                else if (nTemp<16)
                    XBuf = psi->XbufTmp2->bufTemp[nTemp-8][kStart + m];
                else
                    XBuf = psi->XBuf[nTemp][kStart + m];
    #endif
        ...
        for (p = 0; p < n; p++) {
...
#ifdef AAC_ENABLE_INTERNAL_SD
            if (i<8)
                XBuf = psi->XbufTmp1->bufTemp[i][mStart];
            else if (i < 16)
                XBuf = psi->XbufTmp2->bufTemp[i-8][mStart];
            else
            ...
            for (m = mStart; m < mEnd; m++) {
                psi->eCurr[m - kStart] = t;
                psi->eCurrExp[m - kStart] = nScale + 1;    /* +1 for invFact = Q31 */
            }
            if (psi->eCurrExp[mStart - kStart] > expMax)
                expMax = psi->eCurrExp[mStart - kStart];
        }
    }
    ...
    if (sbrHdr->interpFreq) {
        for (m = 0; m < sbrFreq->numQMFBands; m++) {
            ...
            for (i = iStart; i < iEnd; i++) {
                /* scale to int before calculating power (precision not critical, and avoids overflow) */
                xre = (*XBuf) >> FBITS_OUT_QMFA;
                xim = (*(XBuf+1)) >> FBITS_OUT_QMFA;
                asm ("l.mac %0, %1" : : "%r"(xre), "r"(xre));
                asm ("l.mac %0, %1" : : "%r"(xim), "r"(xim));
    #ifdef AAC_INTERNAL_SRAM_FLOW
                nTemp +=1;
                XBuf = psi->XBuf[nTemp][kStart + m];    
    #else
            ...
            asm volatile("l.mfspr %0, r0, 0x2801" :  "=r"(eCurr.r.lo32));
            asm volatile("l.macrc %0, 32" :  "=r"(eCurr.r.hi32));
        ...
        for (p = 0; p < n; p++) {
            mStart = freqBandTab[p];
            mEnd = freqBandTab[p+1];
            if ( (mEnd-mStart)*2 > sizeof(psi->XBuf) )
            {
                return ERR_AAC_SBR_BITSTREAM;
            }
            for (i = iStart; i < iEnd; i++) {
...
static void CalcMaxGain(PSInfoSBR *psi, SBRHeader *sbrHdr, SBRGrid *sbrGrid, SBRFreq *sbrFreq, int ch, int env, int lim, int fbitsDQ)
{
    ...
    for (m = mStart; m < mEnd; m++) {
        ...
        if (m == freqBandTab[envBand + 1] - kStart) {
            envBand++;
...
static void CalcComponentGains(PSInfoSBR *psi, SBRGrid *sbrGrid, SBRFreq *sbrFreq, SBRChan *sbrChan, int ch, int env, int lim, int fbitsDQ)
{
    ...
    for (m = mStart; m < mEnd; m++) {
        if (m == sbrFreq->freqNoise[psi->noiseFloorBand + 1] - kStart) {
        ...
        if (m == sbrFreq->freqHigh[psi->highBand + 1] - kStart)
            psi->highBand++;
        if (m == freqBandTab[psi->sBand + 1] - kStart) {
            psi->sBand++;
        ...
        if (env == sbrGrid->numEnv - 1) {
            if (m + kStart == r)
                sbrChan->addHarmonic[0][m + kStart] = sbrChan->addHarmonic[1][psi->highBand];
            else
                sbrChan->addHarmonic[0][m + kStart] = 0;
        }
...
static void MapHF(PSInfoSBR *psi, SBRHeader *sbrHdr, SBRGrid *sbrGrid, SBRFreq *sbrFreq, SBRChan *sbrChan, int env, int hfReset)
{
    ...
    for (i = iStart; i < iEnd; i++) {
        ...
        nTemp = i + HF_ADJ;
        if (i + HF_ADJ<8)
           XBuf = psi->XbufTmp1->bufTemp[i + HF_ADJ][kStart];
        else if (i + HF_ADJ<16)
            XBuf = psi->XbufTmp2->bufTemp[i + HF_ADJ -8][kStart];
        else
            XBuf = psi->XBuf[i + HF_ADJ][kStart];
#else
        /* see 4.6.18.7.6 */
        XBuf = psi->XBuf[i + HF_ADJ][kStart];
        nTemp = i + HF_ADJ;
            ...
            if (env == psi->la || env == sbrChan->laPrev) {
                ...
                if (i - iStart < MAX_NUM_SMOOTH_COEFS) {
                    ...
                    for (j = 0; j < MAX_NUM_SMOOTH_COEFS; j++) {
                        ...
                        gFilt += MULSHIFT32_ADD(sbrChan->gTemp[idx][m], hSmoothCoef[j], sbrChan->qTemp[idx][m], hSmoothCoef[j]);
                        idx--;
                        if (idx < 0)
...
#ifdef AAC_INTERNAL_SRAM_FLOW
            if (kStart+sbrFreq->numQMFBands >=64)
                printf("kStart %d + sbrFreq->numQMFBands %d >=64 \n",kStart,sbrFreq->numQMFBands);
            xre += smre;
            *XBuf++ = xre;
            xim += smim;
            *XBuf++ = xim;
...
int AdjustHighFreq(PSInfoSBR *psi, SBRHeader *sbrHdr, SBRGrid *sbrGrid, SBRFreq *sbrFreq, SBRChan *sbrChan, int ch)
{
    ...
    {
        nResult=EstimateEnvelope(psi, sbrHdr, sbrGrid, sbrFreq, env);
        if (nResult)
            return nResult;
