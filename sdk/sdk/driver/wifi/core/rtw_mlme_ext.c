...
#if defined (CONFIG_AP_MODE) && defined (CONFIG_NATIVEAP_MLME) //James
...
void issue_asocrsp(_adapter *padapter, unsigned short status, struct sta_info *pstat, int pkt_type)
{
	...
	*(fctrl) = 0;
	...
	pframe = rtw_set_fixed_ie(pframe, _CAPABILITY_ , (unsigned char *)&val2, &(pattrib->pktlen));

	status = cpu_to_le16(status);
	pframe = rtw_set_fixed_ie(pframe , _STATUS_CODE_ , (unsigned char *)&status, &(pattrib->pktlen));

	val2 = cpu_to_le16(pstat->aid | BIT(14) | BIT(15));
	pframe = rtw_set_fixed_ie(pframe, _ASOC_ID_ , (unsigned char *)&val2, &(pattrib->pktlen));
				...
				#endif
				{
					if(pmlmeext->retry==0)
					{
						...
						#endif
						{
							...
							#endif
							issue_probereq(padapter, NULL, 0);
							issue_probereq(padapter, NULL, 0);
							issue_probereq(padapter, NULL, 0);
						}
					}
...
void rtw_ap_free_sta(_adapter *padapter, struct sta_info *psta)
{
	_irqL irqL;	
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
	struct mlme_ext_priv	*pmlmeext = &(padapter->mlmeextpriv);
