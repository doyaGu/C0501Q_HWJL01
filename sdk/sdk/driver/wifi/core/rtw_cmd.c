...
_func_enter_;
	...
	while(1)
	{
		//if ((_rtw_down_sema(&(pcmdpriv->cmd_queue_sema))) == _FAIL)
			//break;
		itpSemWaitTimeout(&(pcmdpriv->cmd_queue_sema),100);	
...
static void traffic_status_watchdog(_adapter *padapter)
{
		...
		/*&& !MgntInitAdapterInProgress(pMgntInfo)*/)
	{
		...
		if( pmlmepriv->LinkDetectInfo.NumRxOkInPeriod > 500 ||
			pmlmepriv->LinkDetectInfo.NumTxOkInPeriod > 500 )
		{
			...
			if(pmlmepriv->LinkDetectInfo.NumTxOkInPeriod > 500)
...
_func_enter_;	
	...
	if(pcmd->res == H2C_DROPPED)
	{
		//TODO: cancel timer and do timeout handler directly...
		//need to make timeout handlerOS independent
		_set_timer(&pmlmepriv->assoc_timer, 2);
	}
	else if(pcmd->res != H2C_SUCCESS)
	{				
