...
)
{
		...
		(BOARD_USB_COMBO == pHalData->BoardType))
	{
		...
		{	
			case LED_PIN_GPIO0:
#ifdef CFG_NET_WIFI_TRANS_LED	

				ioctl(wifiLed, ITP_IOCTL_ON, NULL);
#endif
				break;
		...
		(BOARD_USB_COMBO == pHalData->BoardType))
	{
		...
		switch(pLed->LedPin)
		{
			...
			case LED_PIN_GPIO0:
				...
				ioctl(wifiLed, ITP_IOCTL_OFF, NULL);
#endif
				break;
...
//		We dispatch acture LED blink action according to LedStrategy.
//
static void BlinkWorkItemCallback(PLED_871x pLed)
{
	BlinkHandler(pLed);
}
	...
	)
{
	...
	{
	    char cmd[256];
		snprintf(cmd, sizeof(cmd), ":led:%d",CFG_NET_WIFI_TRANS_LED_NUM);
		wifiLed = open(cmd, O_RDONLY);
	    InitLed871x(padapter,&(pledpriv->SwLed0), LED_PIN_GPIO0);
	}
	#endif
}
