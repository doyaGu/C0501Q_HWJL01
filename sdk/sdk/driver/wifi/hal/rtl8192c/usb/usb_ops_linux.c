...
#define RTW_USB_BULKOUT_TIMEOUT 5000*2 //ms
...
static int usbctrl_vendorreq(struct dvobj_priv  *pdvobjpriv, u8 request, u16 value, u16 index, void *pdata, u16 len, u8 requesttype)
{
	...
	if((padapter->bSurpriseRemoved) ||(padapter->pwrctrlpriv.pnp_bstop_trx)){
        ...
        printf("usbctrl_vendorreq:(padapter->bSurpriseRemoved ||adapter->pwrctrlpriv.pnp_bstop_trx)!!!\n");  
		status = -EPERM; 
		goto exit;
	}	
	...
	printf("rtw_malloc tmp_buf=0x%X,len=%d\n",tmp_buf,len);
...
{	
	...
	if(pcontext)
	{
		...
		{
            printf("usb_bulkout_zero_complete usb_free_urb\n");
...
#define URB_NO_TRANSFER_DMA_MAP 0x0004  /* urb->transfer_dma valid on submit */ //James add
...
static int recvbuf2recvframe(_adapter *padapter, _pkt *pskb)
{
	...
	do{		
		...
		//printf("recvbuf2recvframe: rxdesc=offsset 0:0x%08x, 4:0x%08x, 8:0x%08x, C:0x%08x\n",prxstat->rxdw0, prxstat->rxdw1, prxstat->rxdw2, prxstat->rxdw4);
...
//#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,18)) // http://www.mail-archive.com/netdev@vger.kernel.org/msg17214.html
		...
		else
		{
			...
			DBG_8192C("recvbuf2recvframe: dev_alloc_skb fail\n");
			goto _exit_recvbuf2recvframe;
		}
...
static int startWorkround = 0;
static bool iteinitsave = false;
...
static void usb_read_port_complete(struct urb *purb, struct pt_regs *regs)
{
	...
	if(purb->status==0)//SUCCESS
	{
		...
		{	
			...
			if (skb_queue_len(&precvpriv->rx_skb_queue)<=32)
			{
				tasklet_schedule(&precvpriv->recv_tasklet);
			}
			else
			{
			    printf("recv_tasklet busy\n");
			}
...
static void usb_read_port_cancel(struct intf_hdl *pintfhdl)
{
	...
	for(i=0; i < NR_RECVBUFF ; i++)	
	{
		...
		if(precvbuf->purb)	
		{
			...
			printf("usb_read_port_cancel wifi usb_kill_urb\n");
		}		
	...
	printf("usb_read_port_cancel wifi usb_kill_urb2\n");
#endif
}
...
*/
	...
	purb->transfer_flags |= URB_NO_TRANSFER_DMA_MAP;
	purb->transfer_flags |= USB_ZERO_PACKET; //James
...
*/
	...
	if(timeout_ms >= 0) {
		...
		unsigned long expire = msecs_to_jiffies(timeout_ms); 
		int status;
		...
		if (!wait_for_completion_timeout(&pxmitbuf->done, expire)) {
			usb_kill_urb(purb);
			printf("usb_write_port wifi usb_kill_urb\n");
...
static void usb_write_port_cancel(struct intf_hdl *pintfhdl)
{
	...
	for(i=0; i<NR_XMITBUFF; i++)
	{
		for(j=0; j<8; j++)
		{
		        if(pxmitbuf->pxmit_urb[j])
		        {
		                ...
		                printf("usb_write_port_cancel wifi usb_kill_urb\n");
		        }
		}
	...
	{	
		for(j=0; j<8; j++)
		{
		        ...
		        {
		                usb_kill_urb(pxmitbuf->pxmit_urb[j]);
		                printf("usb_write_port_cancel wifi usb_kill_urb 2\n");
		        }
		}
