...
static MMP_UINT16 CapMemTable[CAPTURE_MEM_BUF_COUNT][2] = {
    ...
    {0,     CAP_INPUT_MAX_HEIGHT * 2 								  },//UV1
    {0,     CAP_INPUT_MAX_HEIGHT * 3 + (CAP_INPUT_MAX_HEIGHT >> 1)    },//Y2
    ...
    CAP_CONTEXT *Capctxt)
{
    ...
    {
        Cap_Set_ISP_HandShaking(Capctxt->Cap_ID,ONFLY_MODE, &Capctxt->outinfo);
		Cap_Set_Interrupt_Mode(Capctxt->Cap_ID,CAP_INT_MODE_SYNC_ERR, MMP_TRUE);
    }
    else
    {
        Cap_Set_ISP_HandShaking(Capctxt->Cap_ID,MEMORY_MODE, &Capctxt->outinfo);
        Cap_Set_Interrupt_Mode(Capctxt->Cap_ID,(CAP_INT_MODE_FRAME_END | CAP_INT_MODE_SYNC_ERR), MMP_TRUE);
    }

	// Update Dither Setting
	if (Capctxt->Ditherinfo.EnDither == MMP_TRUE)
		Cap_Set_Enable_Dither(Capctxt->Cap_ID, &Capctxt->Ditherinfo);
    ...
    Cap_Set_Buffer_addr_Reg(Capctxt->Cap_ID,CapMemBuffer[Capctxt->Cap_ID], Capctxt->outinfo.OutAddrOffset);

    // Update Input Pin mux
    Cap_Set_Input_Pin_Mux_Reg(Capctxt->Cap_ID,&Capctxt->inmux_info);

    // Update Output Pin mux
    Cap_Set_Output_Pin_Mux_Reg(Capctxt->Cap_ID,&Capctxt->outpin_info);

    // Update IO Parameter
    Cap_Set_IO_Mode_Reg(Capctxt->Cap_ID,&Capctxt->iomode_info);

    // Update Input Parameter
    Cap_Set_Input_Data_Info_Reg(Capctxt->Cap_ID,&Capctxt->ininfo);

    // Update Hor Scale
    Cap_Set_HorScale_Width_Reg(Capctxt->Cap_ID,&Capctxt->outinfo);

    Cap_Set_Enable_Reg(Capctxt->Cap_ID,&Capctxt->funen);

    // Update RGB to YUV Matrix
    if (Capctxt->funen.EnCSFun)
        Cap_SetRGBtoYUVMatrix_Reg(Capctxt->Cap_ID,&Capctxt->RGBtoYUVFun);

    // Update Color Format
    Cap_Set_Color_Format_Reg(Capctxt->Cap_ID,&Capctxt->YUVinfo);
    ...
    {
        Cap_SetCCMatrix_Reg(Capctxt->Cap_ID,&Capctxt->CCFun);
    ...
    CAP_SetScaleParam_Reg(Capctxt->Cap_ID,&Capctxt->ScaleFun);

    for(index = 0; index < WEIGHT_NUM; index++)
    {
        if(Capctxt->ScaleFun.HCI >= ScaleRatio[index])
        {
            Cap_SetIntScaleMatrixH_Reg(Capctxt->Cap_ID,WeightMatInt[index]);
            break;
        }
...
#ifdef SENSOR_DEV
    Cap_Set_Error_Handleing(Capctxt->Cap_ID,0xFFFF); //1080i/720p  turn off  error handling.
    Cap_Set_Wait_Error_Reset(Capctxt->Cap_ID);
#else
    Cap_Set_Error_Handleing(Capctxt->Cap_ID,0xFFFF);
    Cap_Set_Wait_Error_Reset(Capctxt->Cap_ID);
#endif

    // Update Skip Pattern
    Cap_Set_Skip_Pattern_Reg(Capctxt->Cap_ID,Capctxt->Skippattern, Capctxt->SkipPeriod);
