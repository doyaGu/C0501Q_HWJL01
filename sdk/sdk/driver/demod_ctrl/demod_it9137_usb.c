...
#if CONFIG_DEMOD_DESC_IT9137_USB_DESC
...
typedef void (*UPPERCALLBACK)(uint32_t index, void *extraData);
    ...
    // 32 ~ 47
    #define REG_BASE_GPIO_SEL_MODE      (ITH_GPIO_BASE + 0x94)
    #define REG_GPIO_DATA_OUT           (ITH_GPIO_BASE + 0x40)
    #define REG_GPIO_PIN_DIRECTION      (ITH_GPIO_BASE + 0x48)
    ...
    // 16 ~ 31
    #define REG_BASE_GPIO_SEL_MODE      (ITH_GPIO_BASE + 0x98)
    #define REG_GPIO_DATA_OUT           (ITH_GPIO_BASE + 0x00)
    #define REG_GPIO_PIN_DIRECTION      (ITH_GPIO_BASE + 0x08)
    #define GPIO_SEL_MODE_SHIFT_BITS    ((CFG_GPIO_DEMOD_SUSPEND-16) << 1)
    #define GPIO_PIN_SEL_SHIFT_BITS     CFG_GPIO_DEMOD_SUSPEND
#else
    // 0 ~ 15
    #define REG_BASE_GPIO_SEL_MODE      (ITH_GPIO_BASE + 0x9C)
    #define REG_GPIO_DATA_OUT           (ITH_GPIO_BASE + 0x00)
    #define REG_GPIO_PIN_DIRECTION      (ITH_GPIO_BASE + 0x08)
...
//=============================================================================
static DEMOD_IT9137_INFO   *pIt9133OmegaInfo = 0;
        ...
        do{
            ...
            switch( pDemod_attr->bus_type )
            {
                case DEMOD_BUS_I2C:  
                    ...
                    pIt9137Info->supportType                    = OMEGA_LNA_Config_2;
                    break;
            ...
            if( result || (dev->demodId == 0x3) )    break;
...
dem_msg(1, "\n\ndemodAddr=0x%x, supportType=%d, streamType=%d (%s), ",
    ...
    switch( pIt9137Info->it9130Demodulator.streamType )
    {
        ...
        default:    dem_msg(1, "\n"); break;
            ...
            if( it9137_result )  dem_msg_ex(DEM_MSG_TYPE_ERR,"[it9137_USB_INIT] ERROR = 0x%x \n", it9137_result);
    ...
    void        *extraData)
{
    ...
    if(dev->demodId == 0x3)
    {
        pIt9137Info->it9130Demodulator = pIt9133OmegaInfo->it9130Demodulator;
        pIt9137Info->it9130Demodulator.demodAddr = 0x01;
    }
    ...
    void        *extraData)
{
    ...
    if( pIt9137Info )
    {
        ...
        do{
            ...
            Demodulator_isLocked((Demodulator*)&(pIt9137Info->it9130Demodulator), (dev->demodId == 0x3)? 1 : 0,//0,
                                 &bLock);
...
DEMOD_DESC  DEMOD_DESC_it9137_usb_desc =
{
    "demod it9137 usb",     // char        *name;
    0,                      // struct _DEMOD_DESC_TAG  *next;
    DEMOD_ID_IT9137_USB,    // DEMOD_TYPE_ID           id;
    0,                      // void        *privInfo;
