...
static void urb_destroy(struct kref *kref)
{
    ...
    if(urb->cookies != URB_BUF_COOKIE)
        LOG_ERROR " urb 0x%X cookies error 0x%08X != 0x%08X \n", urb, urb->cookies, URB_BUF_COOKIE LOG_END
...
int usb_submit_urb(struct urb *urb)
{
	...
	if (!urb || !urb->complete)
		return -EINVAL;
	if (urb->hcpriv) {
		ithPrintf("URB %p submitted while active\n", urb);
		return -EBUSY;
	}
	...
	return usb_hcd_submit_urb(urb, 0);
}
