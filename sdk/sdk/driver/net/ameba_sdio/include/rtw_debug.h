...
#define DRIVERVERSION	"v01"
#define DRIVERVERSION_FREERTOS	"11407_2015_07_27_inic_8195a_gspi_freertos"
...
#undef _seqdump
	...
	#define _dbgdump(fmt, arg...) printf(fmt"\r\n", ##arg)
	#define _seqdump(sel, fmt, arg...) _dbgdump(fmt, ##arg)
#endif
...
#define DBG_871X_LEVEL(level, fmt, ...)     \
    do {\
...
#if defined(WIN32)
#define _DBG_871X_LEVEL(level, fmt, ...)	   \
    do {\
    if (level <= GlobalDebugLevel) {\
    if (level <= _drv_err_ && level > _drv_always_) \
    _dbgdump("ERROR " fmt, __VA_ARGS__); \
            else \
...
#if defined(WIN32)
#define DBG_871X_SEL(sel, fmt, ...) \
    do {\
    if (sel == RTW_DBGDUMP)\
    _DBG_871X_LEVEL(_drv_always_, fmt, __VA_ARGS__); \
        else {\
    if (_seqdump(sel, fmt, __VA_ARGS__)) /*rtw_warn_on(1)*/; \
    } \
    }while(0)

    /* dump message to selected 'stream' with driver-defined prefix */
#define DBG_871X_SEL_NL(sel, fmt, ...) \
    do {\
    if (sel == RTW_DBGDUMP)\
    DBG_871X_LEVEL(_drv_always_, fmt, __VA_ARGS__); \
        else {\
if (_seqdump(sel, fmt, __VA_ARGS__)) /*rtw_warn_on(1)*/; \
    } \
    }while(0)
...
extern void DumpForOneBytes(u8 *pData, u8 Len);
extern void Dump_drv_version(void);
