...
*/
	do{
		command[0] = JEDEC_READ_ID;
	    if (mmpSpiPioRead(norObject->port,command, 1, 0, 0, &id2, 3) == false)
	    {
	    	result = false;
	    	break;
	    }
	    ...
	    command[0] = READ_ID_CMD;
		if (mmpSpiPioRead(norObject->port,command, 1, 0x0, 3, &id1, 2) == false)
		{
			result = false;
			break;
		}
	...
	NOR_VENDOR_CONTEXT*	context)
{
    ...
    case MX_25L25735F:
        ...
        norObject->context.use4BytesAddress	= false;
    	break;
    ...
    uint32_t		outDataBufferSize)
{
    ...
    case ERASE_ALL:
        ...
        result = mmpSpiPioWrite(norObject->port, commands, 1, NULL, 0, 0, 0);
        break;
    ...
    case ERASE_SECTOR:
		...
		{
			result = mmpSpiPioWrite(norObject->port, commands, 1, &arg, 4, 0, 0);
		}
		else
		{
			result = mmpSpiPioWrite(norObject->port, commands, 1, &arg, 3, 0, 0);
		}
#else
		if ( norObject->context.use4BytesAddress == true )
		{
        ...
        if ( norObject->context.use4BytesAddress == true )
        {
			...
			while(remainsize)
			{
				...
				if (cnt) //unalign address
				{
					{
							cmd[0] = WRITE_EN_CMD;
	        				result = mmpSpiPioWrite(norObject->port, cmd, 1, NULL, 0, 0, 0);
					}
				
					//printf("cnt = %d, wptr=0x%x, waddr=0x%x\n", cnt, wptr, waddr);
					result = mmpSpiPioWrite(norObject->port, commands, 1, &waddr, 4, wptr, 4-cnt);
					wptr+=(4-cnt);
					remainsize-=(4-cnt);
					outDataBufferSize-=(4-cnt);
					waddr+=(4-cnt);
					
					//printf("remainsize=%d, wpaddr=0x%x, waddr=0x%x\n", remainsize, wptr, waddr);
				}
				else //align address
				{
					if (remainsize >= 4)
					{
						...
						ret++;
						//printf("dma wptr=0x%x, waddr = 0x%x, wDMASize=0x%x ret=%d\n", wptr, waddr, wDMASize, ret);
						result = mmpSpiDmaWrite(norObject->port, commands, 1, &waddr, 4, wptr, wDMASize);
						
						wptr+= wDMASize;
						waddr+=wDMASize;
						
						//printf("--after DMA wpt = 0x%x remainsize=%d--\n", wptr, remainsize);
					}
					else
					{
						...
						//printf("----- 4 byte  AXI Pio Write  wptr =0x%x, waddr = 0x%x, remainsize=%d----\n", wptr, waddr, remainsize);
						result = mmpSpiPioWrite(norObject->port, commands, 1, &waddr, 4, wptr, remainsize);
						waddr+=	remainsize;
						wptr+=remainsize;
						remainsize = 0;
					}
        ...
        else
        {
			...
			while(remainsize)
			{
				...
				if (cnt) //unalign address
				{
					...
					//printf("cnt = %d, wptr=0x%x, waddr=0x%x\n", cnt, wptr, waddr);
					result = mmpSpiPioWrite(norObject->port, commands, 1, &waddr, 3, wptr, 4-cnt);
				...
				else //align address
				{
					if (remainsize >= 4)
					{
						...
						ret++;
						//printf("dma wptr=0x%x, waddr = 0x%x, wDMASize=0x%x ret=%d\n", wptr, waddr, wDMASize, ret);
						result = mmpSpiDmaWrite(norObject->port, commands, 1, &waddr, 3, wptr, wDMASize);
					...
					else
					{
						...
						//printf("----- AXI Pio Write  wptr =0x%x, waddr = 0x%x, remainsize=%d----\n", wptr, waddr, remainsize);
						result = mmpSpiPioWrite(norObject->port, commands, 1, &waddr, 3, wptr, remainsize);
						waddr+=	remainsize;
						wptr+=remainsize;
						//readsize+=remainsize;
						remainsize = 0;
...
/*
			...
			uint32_t waddr = arg;
			uint8_t  *wptr = (uint8_t*)outDataBuffer;
			//printf("================= outDataBufferSize = %d\n", outDataBufferSize);
			while (remainsize)
			{
				if (remainsize % 4 == 0)
				{
					wsize = remainsize;
					result = mmpSpiDmaWrite(norObject->port, commands, 1, &waddr, 3, wptr, outDataBufferSize);
					if (result)
					{
						remainsize -= wsize;
						wptr += wsize;
				...
				else
				{
					if (remainsize > 4)
					{
						wsize = (outDataBufferSize >> 2) << 2;
						printf("@@ wsize = %d @@\n", wsize);
						result = mmpSpiDmaWrite(norObject->port, commands, 1, &waddr, 3, waddr, wsize);
					...
					else
					{
						wsize = remainsize;
						printf("## wsize = %d ##\n", wsize);
						result = mmpSpiPioWrite(norObject->port, commands, 1, &waddr, 3, wptr, wsize);
						if (result)
						{
							remainsize -= wsize;
							wptr += wsize;
...
*/
#else
            if(outDataBufferSize >= 32 /*g_norAddrMap.bytesPerPage*/)
            {
    			result = mmpSpiDmaWrite(norObject->port, commands, 1, &arg, 3, (uint8_t*)outDataBuffer, outDataBufferSize);
            }
            else
            {
                result = mmpSpiPioWrite(norObject->port, commands, 1, &arg, 3, (uint8_t*)outDataBuffer, outDataBufferSize);
    		}
#endif			
        }
#else
    ...
    case READ:
        ...
        if ( norObject->context.use4BytesAddress == true )
        {
           if(outDataBufferSize >= 16 || (outDataBufferSize&1))
           {           	   
               result = mmpSpiDmaRead(norObject->port, commands, 1, &arg, 4, (uint8_t*)outDataBuffer, outDataBufferSize);
           }
           else
           {
               result = mmpSpiPioRead(norObject->port, commands, 1, &arg, 4, (uint8_t*)outDataBuffer, outDataBufferSize);
           }
        }
        else
        {
			...
			uint32_t raddr = arg;
			uint32_t rsize = 0;
			uint8_t  *rptr = (uint8_t*)outDataBuffer;
			printf("== 1. rptr addr =%x raddr =0x%x ==\n", rptr, raddr);

			//uint8_t*	readBackBuffer = (uint8_t*)memalign(32, 65532);

			uint8_t*	writeBuffer = (uint8_t*)memalign(32, rsize);
			uint32_t    i;
			//static int cnt = 0;	
			...
			while (remainsize)
			{
				...
				{
					rsize = remainsize;
					result = mmpSpiDmaRead(norObject->port, commands, 1, &raddr, 3, rptr, rsize);
				...
				else
				{
					if (remainsize > 4)
					{
						rsize = (outDataBufferSize >> 2) << 2;
						printf("@@ rsize = %d @@\n", rsize);
						result = mmpSpiDmaRead(norObject->port, commands, 1, &raddr, 3, rptr, rsize);
						printf("== 2. rptr addr =%x raddr = %x ==\n", rptr, raddr);

						//mmpSpiDmaRead(norObject->port, commands, 1, &raddr, 3, readBackBuffer, 65532);
						//if (cnt == 1)
						//{
							//for (i= 0; i<32; i++)
							//{
							//	printf("buf[%d]=0x%x\n", raddr+i, *(rptr+i));
							//	printf("2. buf[%d]=0x%x\n", raddr+i, *(readBackBuffer+i));
							//}	
						//}
					...
					else
					{
						rsize = remainsize;
						printf("## rsize = %d ##\n", rsize);
						printf("== 3. rptr addr =%x addr = %x  ==\n", rptr, raddr);
						result = mmpSpiPioRead(norObject->port, commands, 1, &raddr, 3, rptr, rsize);
						if (result)
						{							
							remainsize -= rsize;
							rptr += rsize;
            ...
            {
                result = mmpSpiDmaRead(norObject->port, commands, 1, &arg, 3, (uint8_t*)outDataBuffer, outDataBufferSize);
            }
            else
            {
                result = mmpSpiPioRead(norObject->port, commands, 1, &arg, 3, (uint8_t*)outDataBuffer, outDataBufferSize);
            }
#endif			
        }
#else
    ...
    case READ_STATUS:
        ...
        mmpSpiPioRead(norObject->port, commands, 1, 0, 0, (uint8_t*)outDataBuffer, outDataBufferSize);
        break;

    case READ_ID:
        //9Fh (M7-M0) (ID15-ID8) (ID7-ID0)
        commands[0] = JEDEC_READ_ID;
        result = mmpSpiPioRead(norObject->port, commands, 1, 0, 0, (uint8_t*)outDataBuffer, 5);
        break;
    ...
    case READ_DEVICE:
		...
		commands[3] = arg >> 8;
		commands[4] = arg;
        result = mmpSpiPioRead(norObject->port, commands, 1, &arg, 3, (uint8_t*)outDataBuffer, 2);
		break;
	case EN4B_MODE:
		//B7h
		cmd = EN4B_MODE;
		commands[0] = cmd;
		result = mmpSpiPioWrite(norObject->port, commands, 1, 0, 0, 0,0);
		break;
    default:
