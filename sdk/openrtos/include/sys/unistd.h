...
#if defined(__CYGWIN__) || defined(__rtems__) || defined(__sh__) || defined(__SPU__) || defined(__OPENRTOS__)
#if !defined(__INSIDE_CYGWIN__)
...
extern int itpRead(int file, char *ptr, int len);
extern int itpWrite(int file, char *ptr, int len);

static inline int read(int __fd, void *__buf, size_t __nbyte)
{
    return itpRead(__fd, (char*)__buf, __nbyte);
}

static inline int write(int __fd, const void *__buf, size_t __nbyte)
{
    return itpWrite(__fd, (char*)__buf, __nbyte);
}
