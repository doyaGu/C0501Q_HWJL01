...
function updateCtrlBoardSetting() {
	...
	var $wifi_mode = "client";
    switch(document.getElementById("wifi_mode").selectedIndex) {
        case 0:
            $wifi_mode = "client";
            break;
        case 1:
    ...
    if (confirm("Do you want to update setting?")==true) {
        ...
        var nocache = "&nocache=" + Math.random() * 1000000;
        var hostStr = "http://" + location.host + "/dev/info.cgi?action=setting&brightness=" + $brightness + "&screensaver_time=" + $screensaver_time + "&screensaver_type=" + $screensaver_type + "&lang=" + $lang + "&wifi_mode=" + $wifi_mode + "&keylevel=" + $keylevel + "&keysound_type=" + $keysound_type + "&play_lev=" + $play_lev + nocache;
