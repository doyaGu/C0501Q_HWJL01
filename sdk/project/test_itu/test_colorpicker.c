...
static bool OnSelection(ITUWidget* widget, char* param)
{
    uint32_t value = (uint32_t)atoi(param);
    printf("OnSelection: value=0x%X\n", value);
    return true;
}
...
int SDL_main(int argc, char *argv[])
{
    ...
    window = SDL_CreateWindow("ITU ColorPicker Test",
    ...
    ituSceneLoadFile(&scene, CFG_PRIVATE_DRIVE ":/colorpicker.itu");
