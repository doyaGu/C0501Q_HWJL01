...
static void ListBoxOnLoadPage(ITUListBox* listbox, int pageIndex)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        sprintf(buf, "%d", ((pageIndex - 1) * count + i) % 4);
...
int SDL_main(int argc, char *argv[])
{
    ...
    window = SDL_CreateWindow("ITU icon list box Test",
    ...
    ituSceneLoadFile(&scene, CFG_PRIVATE_DRIVE ":/iconlistbox.itu");
    ...
    // get listbox
    listbox = (ITUListBox*)ituSceneFindWidget(&scene, "IconListBox1");
    if (listbox)
    {
