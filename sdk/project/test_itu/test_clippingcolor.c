...
static ITUBackground* bg;
static ITUWheel* wheel;
...
static void WheelDraw(ITUWidget* widget, ITUSurface* dest, int x, int y, uint8_t alpha)
{
    int clipx, clipy;
    ...
    ituWheelDraw(widget, dest, x, y, alpha);

    ituWidgetSetClipping(widget, dest, x, y, &prevClip);

    ituWidgetGetGlobalPosition(bg, &clipx, &clipy);
    ituSurfaceSetClipping(dest, clipx, clipy, ituWidgetGetWidth(bg), ituWidgetGetHeight(bg));

    for (node = widget->tree.child; node; node = node->sibling)
        ituWidgetSetColor((ITUWidget*)node, 255, 255, 255, 0);

    ituWheelDraw(widget, dest, x, y, alpha);

    for (node = widget->tree.child; node; node = node->sibling)
        ituWidgetSetColor((ITUWidget*)node, 255, 0, 0, 0);
...
int SDL_main(int argc, char *argv[])
{
    ...
    window = SDL_CreateWindow("ITU Clipping Color Test",
    ...
    ituSceneLoadFile(&scene, CFG_PRIVATE_DRIVE ":/clippingcolor.itu");
    ...
    bg = (ITUBackground*)ituSceneFindWidget(&scene, "Background2");

    wheel = (ITUWheel*)ituSceneFindWidget(&scene, "Wheel2");
    ituWidgetSetDraw(wheel, WheelDraw);
