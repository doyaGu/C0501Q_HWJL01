...
static ITUCircleProgressBar* bar = NULL;
...
static void UpdateCircleProgressBar(void)
{
    if (bar)
    {
        static int count = 0;
        ...
        if (count++ >= 100 / 33)
        {
            int value = bar->value >= 100 ? 0 : bar->value + 1;
            ituCircleProgressBarSetValue(bar, value);
            count = 0;
        }
    }
}
...
int SDL_main(int argc, char *argv[])
{
    ...
    window = SDL_CreateWindow("ITU CircleProgressBar Test",
    ...
    ituSceneLoadFile(&scene, CFG_PRIVATE_DRIVE ":/circleprogressbar.itu");
    ...
    bar = (ITUCircleProgressBar*)ituSceneFindWidget(&scene, "CircleProgressBar1");
