...
int SDL_main(int argc, char *argv[])
{
    ...
    window = SDL_CreateWindow("ITU JPEG Test",
    ...
    ituSceneLoadFile(&scene, CFG_PRIVATE_DRIVE ":/jpeg.itu");
    ...
    // get jpeg icon to load
    icon = (ITUIcon*)ituSceneFindWidget(&scene, "Icon1");
    if (icon)
    {
    #if 1
        ituIconLoadJpegFileSync(icon, CFG_PRIVATE_DRIVE ":/icon.jpg");
    #else
        FILE* f = fopen(CFG_PRIVATE_DRIVE ":/icon.jpg", "rb");
        if (f)
        {
            uint8_t* data;
            int size;
