...
function doAjaxSendRequestUpgradeNetRes() {
    var nocache = "&nocache=" + Math.random() * 1000000;
    var hostStr = "http://" + location.host + "/dev/info.cgi?action=upgrade_net_res" + nocache;
    ajaxSendRequest(hostStr, false);
}

function upgradeNetRes() {
    if ((confirm("Do you want to upgrade data by network?")==true)) {
        setTimeout("doAjaxSendRequestUpgradeNetRes()", 5);
        count = 20;
        var counter = setInterval(countDown, 1000);
        return true;
    } else {
...
function doAjaxSendRequestUpgradeNetFw() {
    var nocache = "&nocache=" + Math.random() * 1000000;
    var hostStr = "http://" + location.host + "/dev/info.cgi?action=upgrade_net_fw" + nocache;
    ajaxSendRequest(hostStr, false);
}

function upgradeNetFw() {
    if ((confirm("Do you want to upgrade firmware by network?")==true)) {
        setTimeout("doAjaxSendRequestUpgradeNetFw()", 5);
        count = 20;
        var counter = setInterval(countDown, 1000);
        return true;
    } else {
...
function updateElevatorSetting() {
    if(lockUI)
        return;
    ...
    var $demo_enable = document.getElementById("demo_enable").selectedIndex;
    if ($demo_enable < 0) {
        $demo_enable = 0;
    }

    var $date_format = document.getElementById("date_format").selectedIndex;
    if ($date_format < 0) {
        $date_format = 0;
    }

    var $sound_enable = document.getElementById("sound_enable").selectedIndex;
    if ($sound_enable < 0) {
        $sound_enable = 0;
    }
    ...
    var $info1_format = document.getElementById("info1_format").selectedIndex;
    if ($info1_format < 0) {
        $info1_format = 0;
    }
    ...
    if (confirm("Do you want to update setting?")==true) {
        ...
        var nocache = "&nocache=" + Math.random() * 1000000;
        var hostStr = "http://" + location.host + "/dev/info.cgi?action=setting&lang=" + $lang + "&demo_enable=" + $demo_enable + "&date_format=" + $date_format + "&sound_enable=" + $sound_enable + "&screensaver_time=" + $screensaver_time + "&info1_format=" + $info1_format + nocache;
...
function updateElevatorAccount() {
    if(lockUI)
        return;
    ...
    if (confirm("Do you want to update account?")==true) {
        lockUI = true;

        var nocache = "&nocache=" + Math.random() * 1000000;
        var hostStr = "http://" + location.host + "/dev/info.cgi?action=account&user_id=" + $user_id + "&user_password=" + $user_password+ "&old_password=" + $old_password + nocache;
...
function updateElevatorDateTime() {
    if(lockUI)
        return;
    ...
    var $time_minute = document.getElementById("time_minute").value;
    ...
    if (confirm("Do you want to update date/time?")==true) {
        lockUI = true;

        var nocache = "&nocache=" + Math.random() * 1000000;
        var hostStr = "http://" + location.host + "/dev/info.cgi?action=datetime&date_year=" + $date_year + "&date_month=" + $date_month + "&date_day=" + $date_day + "&time_hour=" + $time_hour + "&time_minute=" + $time_minute + "&time_second=" + $time_second + nocache;
